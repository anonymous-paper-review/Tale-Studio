...
| **v1.1** | 2026-09-07 | 오너 결정 반영 — 피부 hex 캐릭터 소속(`재질.피부.톤` → `톤 처리`), `인물.눈.눈썹 투과` 신설, 좌/우 규약·캐릭터 값 우선 규칙(가이드 §4 10·11), 선택형 승격 5(액센트 입도·곡면 처리·내부선 표시 방식·접지 방식·헤어 하이라이트; 근거 = 오너 참조 사진 `dev/Image_Style/facet_reference/`), 컴파일 규칙 §6. 리프 240 · 선택형 28. **사이클 2**(같은 3장, v1.1 + §6 컴파일 스펙, 무참조 생성)와 **보드 A/B**(확장 보드 vs 보드 없음, `dev/Image_Style/board_ab/`) 실행 — 결과는 각 폴더 synthesis.md |
...
| 버전 | 날짜 | 내용 |
|---|---|---|
| v0.2 | 2026-09-07 | **캐릭터 사이클 1** 반영 — refer1·2·3 + 오너 예시 이미지, 카드 11장(Codex), 제안 55건 → 후보 갱신 12 · 새 facet 19 · 중복 규칙 3, 리프 285 → 304 (`dev/Image_Style/facet_character/cycle1_synthesis.md`, `apply_cycle1.py`) |
| v0.1 | 2026-09-07 | 오너 결정: 페이스페인트 영구/착탈 분리, 종족 파츠 후보 14 정렬, `_기준계` 규약 리프, 다중 폼 별도 카드 |
...
## 앵커 보드 폐지 — 확정 레시피 = 원작 직접 참조 (2026-09-08, 오너 확정)

보드 A/B(`dev/Image_Style/board_ab/`) 결과에 따라 SKILL.md 단계 3~5를 고쳤다: 프로브는 `[원작 Core]` 단일 참조(첫 참조 문장만), 인물 속성·배경 표면 지정 필수, 부정 절은 구조 서술. 보드는 `templates/archive/`에 보관(최종판 2×2 확장 시트 + 원판 정물)하고, 무인물 원작의 인물 프로브처럼 원작에 없는 어휘가 필요할 때만 `[원작, 보드]` 순서 + QA 게이트로 선택 사용.
