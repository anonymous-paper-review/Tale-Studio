...
>
> **다음 세션 진입 시 권장 액션**: (a) [c] 검증 (b) 신규 작업 (c) 잡힌 버그 수정 (d) `specs/changes/` 진행
> 진행 중 changes/: `rollback-artist-card`, `redesign-director-canvas`, `director-storyboard` (`specs/changes/` 참조)
...
### Phase 10 (L0 노드 그래프) — superseded 2026-06-04

Phase 10의 `[c]` 항목 전체가 `rollback-artist-card` change로 superseded됨. 노드 그래프 코드베이스(`canvas-store`, nodes/, edges/ 등)는 제거. Phase 10 상세 히스토리는 아카이브 참조: `specs/archive/2026-06-04-redesign-l0-canvas/`.
...
- 노드 위치 좌표는 shots/scenes JSONB 컬럼 (#15) — 별도 layout 테이블 안 만듦
