...
> (옛 L0~L3 상세 spec 문서는 코드와 중복되어 폐기됨. 미구현 forward 설계만 `director.md`에 남는다.)
>
...
| **Director Canvas** (P4) | 노드 그래프 연출 (샷↔영상 take) | `shots`/`video_clips` + `canvas_position` | `src/features/director/`, `src/stores/director-store.ts` — 설계: [`director.md`](director.md) |
...
- 아직 구현 안 된 forward 설계 → 해당 change(`specs/changes/`) 또는 `director.md`.
