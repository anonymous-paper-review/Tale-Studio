...
> Spec: `specs/layers/director.md`. 관련 change: `redesign-director`(D-5 영상 생성 wire-up과 연속).
> 마커: `[ ]` 미착수 / `[c]` 코드완료·브라우저 검증 대기 / `[x]` 검증완료 / `[~]` 보류.

## Active

### ST-1: 데이터 모델 + 마이그레이션 (2026-06-04 코드 완료)
- [x] `src/types/director.ts` — `ShotNodeData`에 `storyboardImage: StoryboardImage | null`
...
- [x] `director-store.ts` `makeShotData` 기본값: `storyboardImage: null`, `generationMethod: 'T2V'`
...
- [c] `viewMode: 'node' | 'storyboard'` 상태(director-store) + `setViewMode` + persist partialize
...
- [~] Writer 시간 축 연출 정보(움직임/카메라 동선) I2V 프롬프트 투입 — **보류**: director Shot에 movement 필드
...
- redesign-director의 D-4(Writer↔Director sync)가 선행되면 샷 자동 생성이 매끄럽지만, 본 change는
