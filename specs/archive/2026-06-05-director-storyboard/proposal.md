...
Director Canvas(노드 그래프 재설계, `redesign-director`)는 Scene/Shot/Video 노드 골격까지
...
기존 `redesign-director`의 D-5(영상 생성 wire-up)는 "Shot NodePopup 생성 버튼 → generate-video"
...
- Affected specs: `specs/layers/director.md` (Shot 노드 모델, 생성 흐름, 그리드 뷰 섹션 추가)
- Affected code:
  - `src/types/director.ts` (필드 추가)
  - `src/stores/director-store.ts` (액션 + persist + 기본값)
...
- Affected stores: `director-store`, 읽기 의존 `asset-storage-store`(actor/world 이미지 URL)
...
### 영상 생성 (ST-4)
- ⚠️ **Writer 시간 축 연출 정보(움직임/카메라 동선) 투입 보류** — director Shot에 movement 필드가 없고
...
- duration 미전달(director Shot에 없음 → 라우트 기본값). 폴링 5초 간격/5분 타임아웃(legacy 동일).
...
- source-of-truth spec(`director.md`) 본문에 final state 반영
