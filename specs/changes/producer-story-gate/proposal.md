...
- `relationships[]` + `subtext_notes[]`: characters 행이 아닌 프로젝트 차원 데이터 → **별도 정규화 테이블**(`character_relationships`, `subtext_notes`)에 저장(결정 3). 오픈 캐스트로 인물이 동적으로 늘어나도 외래키 무결성으로 일관 유지(인물 삭제 시 관계 자동 정리).
...
- **이미지 생성 스텝 이전**: 기존 파이프라인의 레퍼런스 이미지(view_main/wide_shot) 생성은 writer에서 빠지고, **artist 단계의 초기 생성**으로 이전(명시적·사용자 주도). entity_type 분기(person 턴어라운드 #37 / object 단일 이미지)도 artist에서 수행.
- `renderPrompts`(l5): object도 person과 동일하게 asset ref 주입 — 씬에 등장하는 사물의 샷 간 consistency 보장 (full cast 참여 결정).

### 5. Artist 영향

- **이미지 생성 전담 + Lock UI 제거**: 캐릭터/사물 이미지의 초기 생성·반복이 artist 단계로 일원화(명시적·사용자 주도, 호출마다 과금). 기존 Lock/Unlock 토글(이미지 재생성 차단)은 가치가 없어 제거 — 재생성은 항상 명시적 클릭이고, writer 재실행이 이미지를 안 건드리므로 보호할 대상이 없다.
