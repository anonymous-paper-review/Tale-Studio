...
//
// 샷 선택: **오너 지목** (2026-08-12). 앞선 sh_03_23 판은 시작 그림이 잘못돼 정성 평가가 불가했다.
//   sh_04_28 = 씬4 풀샷 4초, 카메라 static + 인물 rolls down(large) — "가파른 모래 언덕 아래로
//   몸을 던져 굴러 내려간다". 카메라가 고정이라 변인이 인물 움직임으로 좁혀지고, 시작(언덕 위)과
//   끝(바닥)의 차이가 커서 중간 앵커가 의미를 갖는다.
//
...
const CONDS = [
  ...
  { key: 'c3', label: '3장 (중간 2초)', mids: [2], useEnd: true },
  { key: 'd4', label: '4장 (중간 1·3초)', mids: [1, 3], useEnd: true },
  { key: 'e5', label: '5장 (1초마다 전부)', mids: [1, 2, 3], useEnd: true },
]
const MID_TIMES = [1, 2, 3]
...
function readM(): M {
  ...
  return {
    purpose: '앵커 밀도 2판(재실행) — 다른 그림체(us_cartoon), 오너 지목 샷 sh_04_28. 앞선 sh_03_23 판은 시작 그림 결함으로 정성 평가 불가',
  ...
  for (let i = 0; i < plan.length; i += CONCURRENT_CAP) {
    ...
    for (const b of batch) {
      try {
        ...
        m.jobs.push({ key: b.key, label: b.label, depth: b.depth, anchors: b.anchors, anchor_times: b.times, prompt: b.prompt, job_id: id, credits: 18 })
        m.credits_spent += 18
        save(m)
