...
// CLI 실행: node detector.mjs
if (import.meta.url === `file://${process.argv[1]}`) {
  const { hookClassification, findings } = runDetector('src')
  console.log('=== 훅 분류 ===')
  for (const [name, v] of Object.entries(hookClassification)) {
    console.log(`${v.safe === true ? '안전' : v.safe === false ? '위험' : '미확인'}\t${name}\t(${v.file})\t${v.reason}`)
  }
  console.log('\n=== 위험/미확인 통로가 조건 목록에 쓰이는 곳 ===')
  for (const fnd of findings) console.log(JSON.stringify(fnd))
  const unsafeCount = findings.filter((x) => x.status === 'unsafe').length
  const unknownCount = findings.filter((x) => x.status === 'unknown').length
  console.log(`\n총 findings: ${findings.length} (unsafe 확정: ${unsafeCount}, unknown: ${unknownCount})`)
}
