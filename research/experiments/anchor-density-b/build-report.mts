...
<header class="page">
  ...
  <p class="standfirst">1판(소녀 스토리)과 <b>같은 포맷</b>으로 그림체만 바꿨습니다. 시작 그림과 끝 그림 사이를 이미지 생성기로 1초마다 채우고, 영상 생성기에 <b>1장 · 2장 · 3장 · 4장 · 5장 · 6장</b>으로 나눠 넣었습니다. 각 조건마다 <b>시작 그림의 깊이 지도를 함께 넣은 짝</b>을 두어 두 배로 돌렸습니다. <b>판정은 하지 않았습니다.</b></p>
...
<section class="block">
  ...
  <div class="callout" style="margin-bottom:14px">
    ...
    작품 <b>${esc(m.project.title)}</b> · 그림체 <b>${esc(m.project.style_anchor)}</b> · 컷 <span class="mono">${esc(m.shot.id)}</span> (${m.shot.duration}초).
    <br><span class="note">고른 이유: 시작·끝 그림이 둘 다 있는 30컷 중 카메라가 움직이는 것은 8컷뿐이고, 그중 이 컷이 <b>공간을 실제로 이동</b>해(몸을 낮춰 배수관으로 들어감) 1판의 질주 컷과 가장 닮았습니다. 시작과 끝의 차이가 커야 중간 앵커가 의미를 갖습니다.</span>
  </div>
...
<section class="block">
  ...
  <details>
    ...
    <div class="body"><pre class="prompt">${esc(samplePrompt)}</pre></div>
  </details>
  <details>
    <summary>프롬프트 전문 — 가장 많은 조건(8장 + 깊이 지도)</summary>
    <div class="body"><pre class="prompt">${esc(maxPrompt)}</pre></div>
  </details>
  <details>
    <summary>중간 프레임을 만들 때 준 지시문</summary>
    <div class="body"><pre class="prompt">${esc((m.mid_frames ?? []).find((f: { prompt?: string }) => f.prompt)?.prompt ?? '')}</pre></div>
  </details>
</section>
