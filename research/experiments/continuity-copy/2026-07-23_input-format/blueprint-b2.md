...
## 5. 재정의 (v2, 2026-07-23 — SNS 실물 확인 후)

오너가 제공한 SNS 워크플로 실물을 뜯어보니, **시트를 영상 모델에 통째로 넣는 건 실제 용법이
아니었다.** 그들의 시트는 ① 사람이 보는 설계 시각화 ② **프레임 수확처**다 — 콘티 시트가 "한 번의
이미지 생성"이라 셀 12개끼리 인물·세계 일관성이 자동으로 유지되고, 그 셀을 잘라 샷별 시작 프레임으로
쓴다. (해석 문서: `../../../references/2026-07-23-sns-seedance-method.md`)

**B2 본체 (교체)** — "시트 생성 → 셀 수확":

```
콘티 시트 1장 생성 (6샷의 시작 프레임을 한 이미지 안에 격자로 — 캐릭터 정본 참조)
   → 셀 6개를 잘라 각 샷의 시작 프레임으로 사용   ← 셀끼리 일관성 공짜 가설
   → 끝 프레임·영상 생성은 Ⓑ1과 동일
```

- **가설(교체판)**: 샷별 개별 생성(Ⓑ1)보다 "한 생성 안 6컷"이 인물·공간 일관성에서 우세하다 —
  즉 Ⓑ1 vs Ⓑ2는 이제 "이미지 계층의 일관성 제작법"을 가르는 비교가 된다.
- **원안(통째 입력)**: β 1회 시도로 강등 — 오작동(격자가 화면에 나옴 등) 여부만 기록하고 판정에서 제외.
- 생성 규모(교체판): 시트 1콜 + 끝 프레임 6콜 + 클립 6콜 (Ⓑ1보다 이미지 5콜 절약).
