...
- **연출 정보(카메라·동작·길이·결말 상태)는 시트에 담지 않는다** — 시트는 프롬프트로 텍스트를
  금지해 그림만 있고, 연출은 I2V 시점에 B1과 글자까지 동일한 `video_prompt`로 따로 던진다.
  통제 변수 원칙: B1 vs B2 비교가 "시작 프레임 출신" 하나로만 갈리게 하기 위함. 연출 노트가
  이미지 안에 글자로 박힌 건 β의 `beta_sheet.jpg`뿐이다.
