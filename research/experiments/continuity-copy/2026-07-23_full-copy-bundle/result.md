...
>
> 작성 2026-07-23 · 상태: **판정 완료 — 무효** (오너, 2026-07-24) · 설계 문서: [`design.md`](design.md) · 원본 컷 분석: [`conti_full.md`](conti_full.md)
>
> **판정(2026-07-24, 오너): 무효 — 블라인드 순위 판정 없이 종결.** 입력 결함 2건이 확인됐다.
> ① **제품판 입력 결함**: 샷 그림이 정본 기반 i2i가 아니라 샷마다 독립 T2I로 생성됐다. 원인은
> 제품 코드의 배선 단절(샷 이미지 스테이지가 찾는 정본 에셋 매니페스트를 만드는 스테이지가
> 리팩토링 때 삭제됨 — 개선 백로그 I9). 스타일·신원·공간이 샷마다 제각각인 입력이 그대로 영상에 들어갔다.
> ② **사람판 테이크 계획 결함**: 원본 도입부(0~5.8초)는 1초 안팎 플래시 컷 8개의 빨리감기 몽타주인데
> 컷마다 독립 4초 테이크를 할당해 도입부가 붕괴했고, 생성 프레임 구도도 원본과 동떨어졌다.
> 재설계: [`../2026-07-24_full-copy-v2/design.md`](../2026-07-24_full-copy-v2/design.md)
...
사람판을 촬영 단위별로 뜯어볼 때는 [`assets/arm-bkm/io_review.md`](assets/arm-bkm/io_review.md) —
테이크 27개 각각에 대해 **넣은 것**(시작·끝 그림)과 **나온 것**(생성 영상의 첫·중간·끝 화면)을
한 장에 세워 대조한 문서다. 어느 테이크가 지시대로 나왔고 어느 테이크가 어긋났는지 여기서 짚는다.
...
2026-07-23_full-copy-bundle/
  ...
  assets/
    ...
    arm-bkm/         사람판 시작·끝 그림 54장 + 검수용 문서(README.md) + 입력↔출력 대조(io_review.md)
