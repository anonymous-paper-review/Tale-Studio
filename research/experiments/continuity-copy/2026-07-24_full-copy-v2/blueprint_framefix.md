...
```
(ORIGIN 시작 프레임) ──「+끝 상태 문장」 i2i──▶ 끝 프레임 ──「시작+끝+모션 프롬프트」 Seedance──▶ 클립
```

- 샷 블록 맨 위 썸네일은 **v1 무효 팔(BASE)이 같은 샷을 T2I로 뽑았던 결과**다 — 이번 실험의 입력이 아니고, "이 샷이 뭘 찍는 샷인지" 감 잡는 참고용. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.
- 각 샷의 영상 API에 실제로 들어가는 것은 블록의 **3단계 리스트가 전부**: 이미지 2장(시작+끝) + 모션 텍스트 1개. ORIGIN 팔과의 차이는 **이미지 ②(끝 프레임) 하나뿐**이다.
- 원본 콘티 썸네일(`assets/thumbs/s*.jpg`)은 이 문서에 절대 싣지 않는다 — 오염 금지 규약(§1) 대상.
...
이 팔은 "제품이 스스로 할 수 있는 업그레이드"를 재는 팔이다. 따라서 이 설계도의 저작과 이후 실행 전 과정에서 **원본 영상 분석 자료를 절대 참조하지 않는다**: `conti_full.md`, `takes.json`, `assets/conti/`, `arm-bkm/` 폴더 열람 금지 (이 문서에 원본 콘티 썸네일 `assets/thumbs/s*.jpg`를 임베드하는 것도 금지). 끝 상태 문장의 유일한 재료는 shots.json의 `video_prompt`·`character_action`·`camera`·`composition` 4개 필드다. (원본을 본 사람의 지식이 새면 이 팔은 BKM과 구분이 안 된다.) `jobs.bkm.json`은 잡 파일 **스키마(필드 구성)** 확인 목적으로만 참조했고, 그 안의 프롬프트 내용은 일절 차용하지 않았다.
...
`ref_identity.jpg` — 캐릭터 신원 정본 참조.

![](assets/thumbs/ref_plate.jpg)

`ref_plate.jpg` — 공간 플레이트 정본 참조.
...
### 샷 01 — shot_1 · 5초 → `clips/arm-framefix/01.mp4`

![](assets/thumbs/base_01.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "Establish the clinical, eerie atmosphere of the retro-pastel restroom at dawn." · 카메라 WS / eye_level / static · 구도 "The vanishing point at the center of the restroom corridor." · 무드 "Desaturated pastels with a cold, clinical blue undertone."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/01.png` — [blueprint_origin.md](blueprint_origin.md) 샷 01의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 A′ (무인)

- 넣는 것: 시작 프레임 `arm-origin/frames/01.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/01_end.png`
...
**3단계 · ▶ 영상 API 입력** — Seedance 2.0 · 5초 · 16:9 · task `i2v_se`

- 이미지 ① (시작): `arm-origin/frames/01.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/01_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/01.mp4`
...
<details><summary>jobs.framefix.json 조각</summary>
...
### 샷 02 — shot_2 · 4초 → `clips/arm-framefix/02.mp4`

(v1 산출 없음 — 차단됐던 샷)

> **shot_2 특례 대상** (§2-6): ORIGIN에서 이 샷이 제외되면 아래 페이로드 2건 모두 실행하지 않는다.

**writer 원문**: 행동 "Introduce the protagonist into the sterile environment, emphasizing her isolation." · 카메라 MFS / eye_level / static · 구도 "The girl as she enters the frame." · 무드 "Maintain the cold dawn light, highlighting the pale blue of the dress."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/02.png` — [blueprint_origin.md](blueprint_origin.md) 샷 02의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 A

- 넣는 것: 시작 프레임 `arm-origin/frames/02.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/02_end.png`
...
**3단계 · ▶ 영상 API 입력** — Seedance 2.0 · 4초 · 16:9 · task `i2v_se`

- 이미지 ① (시작): `arm-origin/frames/02.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/02_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/02.mp4`
...
### 샷 03 — shot_3 · 7초 → `clips/arm-framefix/03.mp4`

![](assets/thumbs/base_03.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "Create suspense by showing the girl's ignorance of the ghostly whisper coming from below." · 카메라 MCU / eye_level / static · 구도 "The girl's eyes in the mirror reflection." · 무드 "Focus on the pink of the lip gloss and the pale blue of her dress reflection."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/03.png` — [blueprint_origin.md](blueprint_origin.md) 샷 03의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 A

- 넣는 것: 시작 프레임 `arm-origin/frames/03.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/03_end.png`
...
**3단계 · ▶ 영상 API 입력** — Seedance 2.0 · 7초 · 16:9 · task `i2v_se`

- 이미지 ① (시작): `arm-origin/frames/03.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/03_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/03.mp4`
...
### 샷 04 — shot_4 · 3초 → `clips/arm-framefix/04.mp4`

![](assets/thumbs/base_04.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "Identify the source of the whisper, grounding the horror in a physical object." · 카메라 ECU / high_angle / static · 구도 "The center of the drain hole." · 무드 "High contrast between the bright sink and the absolute black of the drain."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/04.png` — [blueprint_origin.md](blueprint_origin.md) 샷 04의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 A′ (무인 · 무동작 샷)

- 넣는 것: 시작 프레임 `arm-origin/frames/04.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/04_end.png`
...
**3단계 · ▶ 영상 API 입력** — Seedance 2.0 · 3초 · 16:9 · task `i2v_se`

- 이미지 ① (시작): `arm-origin/frames/04.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/04_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/04.mp4`
...
### 샷 05 — shot_5 · 4초 → `clips/arm-framefix/05.mp4`

![](assets/thumbs/base_05.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "Conclude the scene with the girl's unsettling normalcy, leaving the audience in dread." · 카메라 MS / eye_level / static · 구도 "The girl's face." · 무드 "A slightly colder, more clinical blue tone to end the scene."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/05.png` — [blueprint_origin.md](blueprint_origin.md) 샷 05의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 A

- 넣는 것: 시작 프레임 `arm-origin/frames/05.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/05_end.png`
...
- 이미지 ① (시작): `arm-origin/frames/05.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/05_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/05.mp4`
...
### 샷 06 — shot_6 · 4초 → `clips/arm-framefix/06.mp4`

![](assets/thumbs/base_06.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "Visualize the girl's sudden isolation and the eerie realization of an uncanny presence in the empty space." · 카메라 MCU / eye_level / static · 구도 "The girl's eyes in the mirror reflection." · 무드 "Cool dawn tones with a hint of retro pastel blue to enhance the quiet dread."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/06.png` — [blueprint_origin.md](blueprint_origin.md) 샷 06의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 A

- 넣는 것: 시작 프레임 `arm-origin/frames/06.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/06_end.png`
...
- 이미지 ① (시작): `arm-origin/frames/06.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/06_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/06.mp4`
...
### 샷 07 — shot_7 · 3초 → `clips/arm-framefix/07.mp4`

![](assets/thumbs/base_07.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "Clearly identify the source of the whisper, transforming a vague feeling into a specific, localized threat." · 카메라 ECU / high_angle / static · 구도 "The center of the drain grating." · 무드 "High contrast to emphasize the darkness within the drain."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/07.png` — [blueprint_origin.md](blueprint_origin.md) 샷 07의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 A′ (무인 · 무동작 샷)

- 넣는 것: 시작 프레임 `arm-origin/frames/07.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/07_end.png`
...
- 이미지 ① (시작): `arm-origin/frames/07.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/07_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/07.mp4`
...
### 샷 08 — shot_8 · 5초 → `clips/arm-framefix/08.mp4`

![](assets/thumbs/base_08.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "Escalate suspense by showing the character's fatal curiosity as she draws closer to the source of the voice." · 카메라 CU / low_angle / **dolly_in** · 구도 "The girl's ear and her wide, anxious eye." · 무드 "Deepen the cool blues while introducing a faint magenta glow in the shadows."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/08.png` — [blueprint_origin.md](blueprint_origin.md) 샷 08의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 B (dolly_in 종착)

- 넣는 것: 시작 프레임 `arm-origin/frames/08.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/08_end.png`
...
- 이미지 ① (시작): `arm-origin/frames/08.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/08_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/08.mp4`
...
### 샷 09 — shot_9 · 3초 → `clips/arm-framefix/09.mp4`

![](assets/thumbs/base_09.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "Capture the peak of the girl's curiosity and tension as she investigates the source of the sound." · 카메라 MS / low_angle / handheld_drift · 구도 "The girl's hands near the pipes" · 무드 "Cold dawn blue tones with high contrast shadows."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/09.png` — [blueprint_origin.md](blueprint_origin.md) 샷 09의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 A (drift는 구도 고정 취급)

- 넣는 것: 시작 프레임 `arm-origin/frames/09.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/09_end.png`
...
- 이미지 ① (시작): `arm-origin/frames/09.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/09_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/09.mp4`
...
### 샷 10 — shot_10 · 2초 → `clips/arm-framefix/10.mp4`

![](assets/thumbs/base_10.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "Shock the audience with a sensory blackout and a brief flash of violence." · 카메라 POV / eye_level / static · 구도 "The center of the frame" · 무드 "Pitch black interrupted by an aggressive, saturated magenta burst."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/10.png` — [blueprint_origin.md](blueprint_origin.md) 샷 10의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 A′ (무인)

- 넣는 것: 시작 프레임 `arm-origin/frames/10.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/10_end.png`
...
**3단계 · ▶ 영상 API 입력** — Seedance 2.0 · 2초 · 16:9 · task `i2v_se`

- 이미지 ① (시작): `arm-origin/frames/10.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/10_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/10.mp4`
...
### 샷 11 — shot_11 · 4초 → `clips/arm-framefix/11.mp4`

![](assets/thumbs/base_11.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "Establish the uncanny presence of the doppelganger and the girl's defeat." · 카메라 WS / high_angle / handheld_drift · 구도 "The doppelganger's standing figure" · 무드 "Desaturated, clinical dawn light with deep blue shadows."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/11.png` — [blueprint_origin.md](blueprint_origin.md) 샷 11의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 A (drift는 구도 고정 취급 · 무동작 샷)

- 넣는 것: 시작 프레임 `arm-origin/frames/11.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/11_end.png`
...
- 이미지 ① (시작): `arm-origin/frames/11.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/11_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/11.mp4`
...
### 샷 12 — shot_12 · 5초 → `clips/arm-framefix/12.mp4`

![](assets/thumbs/base_12.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "Demonstrate the doppelganger's cold, mechanical efficiency in disposing of the original." · 카메라 MFS / eye_level / handheld_drift · 구도 "The doppelganger's hand on the girl's arm" · 무드 "Clinical, muted tones to match the emotionless action."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/12.png` — [blueprint_origin.md](blueprint_origin.md) 샷 12의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 A (drift는 구도 고정 취급)

- 넣는 것: 시작 프레임 `arm-origin/frames/12.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/12_end.png`
...
- 이미지 ① (시작): `arm-origin/frames/12.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/12_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/12.mp4`
...
### 샷 13 — shot_13 · 4초 → `clips/arm-framefix/13.mp4`

![](assets/thumbs/base_13.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "Final reveal of the doppelganger's perfect, terrifying lack of emotion." · 카메라 CU / eye_level / handheld_drift · 구도 "Doppelganger's eyes" · 무드 "High contrast, emphasizing the pale skin and dark hair."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/13.png` — [blueprint_origin.md](blueprint_origin.md) 샷 13의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 A (drift는 구도 고정 취급 · 무동작 샷)

- 넣는 것: 시작 프레임 `arm-origin/frames/13.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/13_end.png`
...
- 이미지 ① (시작): `arm-origin/frames/13.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/13_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/13.mp4`
...
### 샷 14 — shot_14 · 3초 → `clips/arm-framefix/14.mp4`

![](assets/thumbs/base_14.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "To emphasize the physical weight and total lack of life in the victim's body through a grounding floor-level perspective." · 카메라 FS / low_angle / handheld_drift · 구도 "The point of contact between the girl's shoulder and the floor." · 무드 "Cold, clinical dawn light with harsh magenta shadows in the corners."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/14.png` — [blueprint_origin.md](blueprint_origin.md) 샷 14의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 A (drift는 구도 고정 취급)

- 넣는 것: 시작 프레임 `arm-origin/frames/14.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/14_end.png`
...
- 이미지 ① (시작): `arm-origin/frames/14.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/14_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/14.mp4`
...
### 샷 15 — shot_15 · 2초 → `clips/arm-framefix/15.mp4`

![](assets/thumbs/base_15.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "Highlight the uncanny and fetishistic detachment of the antagonist through a close-up of a stolen personal item." · 카메라 ECU / eye_level / handheld_drift · 구도 "The angular toe of the black Mary Jane heel." · 무드 "High contrast with magenta highlights reflecting off the black leather."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/15.png` — [blueprint_origin.md](blueprint_origin.md) 샷 15의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 A (drift는 구도 고정 취급)

- 넣는 것: 시작 프레임 `arm-origin/frames/15.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/15_end.png`
...
- 이미지 ① (시작): `arm-origin/frames/15.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/15_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/15.mp4`
...
### 샷 16 — shot_16 · 4초 → `clips/arm-framefix/16.mp4`

![](assets/thumbs/base_16.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "To build dread through a prolonged moment of unnatural stillness and psychological void." · 카메라 MS / eye_level / handheld_drift · 구도 "The doppelganger's eyes." · 무드 "Desaturated blues and pinks with deep, oppressive shadows."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/16.png` — [blueprint_origin.md](blueprint_origin.md) 샷 16의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 A (drift는 구도 고정 취급 · 무동작 샷)

- 넣는 것: 시작 프레임 `arm-origin/frames/16.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/16_end.png`
...
- 이미지 ① (시작): `arm-origin/frames/16.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/16_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/16.mp4`
...
### 샷 17 — shot_17 · 3초 → `clips/arm-framefix/17.mp4`

![](assets/thumbs/base_17.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "To signify the completion of the 'replacement' and the abandonment of the original girl." · 카메라 MFS / eye_level / **tracking** · 구도 "The doppelganger's back as she walks away." · 무드 "Cold blue dominance with a final flash of magenta from the overhead lights."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/17.png` — [blueprint_origin.md](blueprint_origin.md) 샷 17의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 B (tracking 종착)

- 넣는 것: 시작 프레임 `arm-origin/frames/17.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/17_end.png`
...
- 이미지 ① (시작): `arm-origin/frames/17.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/17_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/17.mp4`
...
### 샷 18 — shot_18 · 2.5초 → `clips/arm-framefix/18.mp4`

![](assets/thumbs/base_18.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "To convey a sense of uncanny detachment by showing the doppelganger's mechanical and indifferent movement as she replaces the original girl." · 카메라 MS / eye_level / static · 구도 "The doppelganger's face" · 무드 "Cool and clinical, emphasizing the pale blue tones to match the dawn light."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/18.png` — [blueprint_origin.md](blueprint_origin.md) 샷 18의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 A

- 넣는 것: 시작 프레임 `arm-origin/frames/18.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/18_end.png`
...
**3단계 · ▶ 영상 API 입력** — Seedance 2.0 · 2.5초 · 16:9 · task `i2v_se`

- 이미지 ① (시작): `arm-origin/frames/18.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/18_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/18.mp4`
...
### 샷 19 — shot_19 · 2.5초 → `clips/arm-framefix/19.mp4`

![](assets/thumbs/base_19.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "To create a vacuum of sound and presence, heightening the dread through the sudden emptiness of the space." · 카메라 WS / eye_level / static · 구도 "The closing door" · 무드 "Desaturated and hollow, emphasizing the lack of life in the room."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/19.png` — [blueprint_origin.md](blueprint_origin.md) 샷 19의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 A′ (무인)

- 넣는 것: 시작 프레임 `arm-origin/frames/19.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/19_end.png`
...
- 이미지 ① (시작): `arm-origin/frames/19.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/19_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/19.mp4`
...
### 샷 20 — shot_20 · 4초 → `clips/arm-framefix/20.mp4`

![](assets/thumbs/base_20.jpg)

(참고) v1 무효 팔이 이 샷을 T2I로 뽑았던 결과 — 이번 입력 아님, 이 샷이 무엇인지 감 잡는 용도. 실제 시작 프레임은 ORIGIN 팔이 새로 만든 것을 공유한다.

**writer 원문**: 행동 "To deliver the final chilling revelation that the original girl is still there, discarded and forgotten." · 카메라 CU / low_angle / **dolly_in** · 구도 "The white crew socks and black Mary Jane heels" · 무드 "The warmest but most ominous tone, with deep shadows creeping from the stall."

**1단계 · 시작 프레임 = ORIGIN 공유 (이 팔에서 생성 없음)**

- `assets/arm-origin/frames/20.png` — [blueprint_origin.md](blueprint_origin.md) 샷 20의 1단계 산출을 그대로 쓴다

**2단계 · 끝 프레임 생성 (i2i)** — 패턴 B (dolly_in 종착 · 무인 시작 → `same scene`)

- 넣는 것: 시작 프레임 `arm-origin/frames/20.png` 1장 + 아래 프롬프트 → `openai/gpt-image-2/edit` (fal)
- 나오는 것: `assets/arm-framefix/frames/20_end.png`
...
- 이미지 ① (시작): `arm-origin/frames/20.png` — 1단계 공유분
- 이미지 ② (끝): `arm-framefix/frames/20_end.png` — 2단계 산출 ← **ORIGIN에 없는 유일한 추가 입력**
- 텍스트 (모션): 아래 원문 — ORIGIN과 동일 문자열
- 산출: `clips/arm-framefix/20.mp4`
