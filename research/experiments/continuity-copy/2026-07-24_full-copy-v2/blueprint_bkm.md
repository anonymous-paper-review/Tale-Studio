...
이 문서의 본체는 §4의 블록 24개(영상 테이크 22 + 스틸 2)다. 파이프라인은 전 테이크 공통으로 한 줄이다:
...
- **s01 ← T05(s13)**: `s01_start.jpg`와 `s13_end.jpg`가 동일 구도다 — 세면대 안 어안에서 올려다본 얼굴, 주황 림이 하단을 감싸고 좌우에 거울·튜브 조명, 얼굴 중앙 상단. s13 테이크의 얼굴 진입 완료 구간을 그대로 쓴다.
  ...
  왼쪽부터: 원본 s01 시작 · 원본 s13 끝 — 동일 구도(세면대 안 어안의 내려다보는 얼굴). 그래서 T05에서 오려도 된다.
...
- **s02 ← T10(s20)**: `s02_start.jpg`는 왼쪽 전방 저위치 카메라가 본 "숙인 얼굴" 초근접(좌하향 3/4 얼굴, 초커 늘어짐, 민트 타일 보케). `s20_start.jpg`와 카메라 측·높이·얼굴 방향·배경이 일치하고 샷 크기만 한 단계 타이트 — T10 하강 초반 구간 + 펀치인 ~20%로 커버.
  ...
  왼쪽부터: 원본 s02 시작 · 원본 s20 시작 — 카메라 측·높이·얼굴 방향·배경 일치, 샷 크기만 한 단계 차이. 펀치인 ~20%로 커버.
...
왼쪽부터: 원본 s09 시작 · s09 끝 — QC 대조 기준 프레임
...
왼쪽부터: 원본 s10 시작 · s10 끝 — QC 대조 기준 · s12 끝(오버숄더 꼬리 — 이 테이크 말미로 커버)
...
왼쪽부터: 원본 s11 시작 · s17 시작 · s17 끝 — QC 대조 기준 프레임
...
원본 s12 시작 — QC 대조 기준 (s12 끝의 오버숄더 꼬리는 T02 담당)
...
왼쪽부터: 원본 s13 시작 · s13 끝 — QC 대조 기준 · s01 시작(도입 재사용 목표) · s23 끝(꼬리 대체 목표)
...
왼쪽부터: 원본 s15 시작 · s15 끝 — QC 대조 기준 · s04 시작(도입 재사용 목표, 펀치인 ~35%)
...
왼쪽부터: 원본 s16 시작 · s16 끝 — QC 대조 기준 프레임
...
왼쪽부터: 원본 s18 시작 · s18 끝 — QC 대조 기준 프레임
...
왼쪽부터: 원본 s19 시작 · s19 끝 — QC 대조 기준 프레임
...
왼쪽부터: 원본 s20 시작 · s20 끝 — QC 대조 기준 · s02 시작(도입 재사용 목표, 펀치인 ~20%)
...
왼쪽부터: 원본 s22 시작 · s22 끝 — QC 대조 기준 프레임 (카운터 위 두 발 = 도플갱어 복선)
...
원본 s23 시작 — QC 대조 기준 (드레인 POV 꼬리인 s23 끝은 T05가 대체 담당)
...
왼쪽부터: 원본 s25 시작 · s25 끝 — QC 대조 기준 프레임 (끌기는 s25에서 이미 시작돼 있다)
...
왼쪽부터: 원본 s26 시작 · s26 끝 — QC 대조 기준 프레임 (두 인물이 좌측 1/3에 작게 — v1이 실패한 바로 그 구도)
...
왼쪽부터: 원본 s27 시작 · s27 끝 — QC 대조 기준 프레임
...
왼쪽부터: 원본 s28 시작 · s28 끝 — QC 대조 기준 · s05 시작(도입 재사용 목표 — 콘티 명시 동일 구도)
...
왼쪽부터: 원본 s29 시작 · s29 끝 — QC 대조 기준 프레임
...
원본 s30 시작 — QC 대조 기준 (s30 끝은 암전이라 대조 제외)
...
### T19 — 컷 s03 커버 · 4초 · 도입 전용 → `clips/arm-bkm2/T19.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s03_start.jpg)

원본 s03 시작 — QC 대조 기준 (s03 끝은 경계 블리드라 대조 제외 — §3-4 발견 ④)
...
- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T19_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T19_end.jpg`
...
- 이미지 ①: `arm-bkm2/frames/T19_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T19_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T19.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s03 0.63~1.20 ← T19 1.5~2.1 (도입 전용)
...
### T20 — 컷 s06 커버 · 4초 · 도입 전용 → `clips/arm-bkm2/T20.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s06_start.jpg) ![](assets/thumbs/s06_end.jpg)

왼쪽부터: 원본 s06 시작 · s06 끝 — QC 대조 기준 프레임
...
- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T20_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T20_end.jpg`
...
- 이미지 ①: `arm-bkm2/frames/T20_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T20_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T20.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s06 1.63~2.73 ← T20 0.5~1.6 (도입 전용)
...
### T21 — 컷 s07 커버 · 4초 · 도입 전용 → `clips/arm-bkm2/T21.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s07_start.jpg) ![](assets/thumbs/s07_end.jpg)

왼쪽부터: 원본 s07 시작 · s07 끝 — QC 대조 기준 프레임
...
- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T21_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T21_end.jpg`
...
- 이미지 ①: `arm-bkm2/frames/T21_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T21_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T21.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s07 2.73~3.87 ← T21 0.5~1.6 (도입 전용)
...
### T22 — 컷 s08 커버 · 4초 · 도입 전용 → `clips/arm-bkm2/T22.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s08_start.jpg) ![](assets/thumbs/s08_end.jpg)

왼쪽부터: 원본 s08 시작 · s08 끝 — QC 대조 기준 프레임
...
- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T22_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T22_end.jpg`
...
- 이미지 ①: `arm-bkm2/frames/T22_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T22_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T22.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s08 3.87~5.77 ← T22 1.0~2.9 (도입 전용)
