...
한 줄 요약: v1 BKM(사람판)을 무효로 만든 결함 2건 — 도입 플래시 오분할·구도 이탈 — 을 고친 재설계판. 도입 플래시 8컷 중 4컷은 후반 동일/유사 구도 테이크에서 오려 쓰고, 대응 없는 도입 4컷은 도입 몽타주 별개 실험으로 이관해 이 팔에서는 생성하지 않으며, 프레임은 전량 재생성하되 테이크마다 원본 프레임을 눈으로 대조한 정밀 구도 문장으로 저작하며, 원본 대응 프레임과의 나란히 대조 QC를 강제한다.

- **상태: 오너 확인 대기** (이 문서의 페이로드로 생성 착수 전 오너 컨펌 필요 — 유료 호출 없음, 문서만 작성된 상태)
- 작성일: 2026-07-24 · 개정 2026-07-24: 도입 몽타주 별개 실험 분리 (오너 확정)
...
이 문서의 본체는 §4의 블록 20개(영상 테이크 18 + 스틸 2)다. 파이프라인은 전 테이크 공통으로 한 줄이다:

> **정본 + 플레이트 —i2i→ 시작 프레임 —i2i→ 끝 프레임 —(시작 + 끝 + 연출 문장)→ 테이크 클립 —편집→ 원본 30컷 타이밍**

각 테이크 블록은 위에서 아래로 이렇게 읽는다:

1. **원본 목표** — 이 테이크가 흉내내야 하는 원본 콘티 프레임(300px 썸네일). QC는 이 그림과 생성 프레임을 나란히 놓고 한다(§6).
2. **i2i 재료** — 실제 입력으로 들어가는 그림 전부. 원본 픽셀은 어떤 테이크에도 입력되지 않는다 — 구도는 텍스트 지시로만 전달(BKM 정의, §1).
3. **1단계·2단계** — 시작/끝 프레임을 만드는 i2i 호출. "넣는 것 → 나오는 것" + 프롬프트 원문(영문 코드블록이 API에 그대로 가는 최종 문자열) + 한국어 번역.
4. **3단계 · ▶ 영상 API 입력** — 시작·끝 프레임 2장 + 연출 문장이 Seedance 2.0에 들어간다. 코드블록이 입력 전문이다(공통 계약 포함, 축약 없음).
5. **편집에서 커버하는 컷** — 완성 클립의 어느 구간을 원본 어느 컷 타이밍에 붙이는지(§3-2 표의 역방향 뷰).

썸네일의 고해상도 원본은 각 블록 말미 "원본 대응 프레임 (QC 대조)" 경로에 있다. 공통 계약(bible·bible_two·negative)의 한국어 전문은 §4-0에 한 번만 있고, 각 테이크 번역 말미의 "(이하 공통 계약)"이 그것을 가리킨다. 실행용 JSON 조각은 각 블록의 접힌 `<details>` 안에 있다.
...
1. **구간설정(편집 계획)**: 도입 플래시는 전용 신규 테이크 생성 금지가 원칙. 후반 동일/유사 구도 테이크의 구간에서 오려 쓴다. 후반에 대응 구도가 정말 없는 컷은 이 팔에서 생성하지 않고 도입 몽타주 별개 실험으로 이관한다(§3, 오너 확정 2026-07-24).
...
정본·플레이트 미리보기 (300px 썸네일 — 원본 경로는 위 목록):

![](assets/thumbs/ref_identity.jpg) ![](assets/thumbs/ref_plate.jpg)

왼쪽부터: 정본(인물 신원) · 플레이트(빈 방 공간)
...
2. 도입 플래시(s01~s08)는 후반 테이크 재사용이 기본. 프레임 대조로 대응 구도를 확정한 4컷(s01·s02·s04·s05)만 재사용하고, 대응이 없는 4컷은 **이 팔에서 생성하지 않는다** — 도입 몽타주 별개 실험([../2026-07-24_intro-montage/design.md](../2026-07-24_intro-montage/design.md))의 산출로 채우는 슬롯으로 비워둔다 (오너 확정 2026-07-24, 기각 대조는 §3-2 근거).
...
| s03 | 0.63~1.20 | 0.6 | — (도입 몽타주 실험 슬롯) | — | **생성 없음** — 몽타주 실험 산출로 채움 |
| s04 | 1.20~1.50 | 0.3 | T06 | 2.2~2.5 | 재사용 + 펀치인 ~35% (얼굴+수전 영역) |
| s05 | 1.50~1.63 | 0.1 | T16 | 0.3~0.4 | 재사용 (콘티 명시 s05=s28) |
| s06 | 1.63~2.73 | 1.1 | — (도입 몽타주 실험 슬롯) | — | **생성 없음** — 몽타주 실험 산출로 채움 |
| s07 | 2.73~3.87 | 1.1 | — (도입 몽타주 실험 슬롯) | — | **생성 없음** — 몽타주 실험 산출로 채움 |
| s08 | 3.87~5.77 | 1.9 | — (도입 몽타주 실험 슬롯) | — | **생성 없음** — 몽타주 실험 산출로 채움 |
...
- **s01 ← T05(s13)**: `s01_start.jpg`와 `s13_end.jpg`가 동일 구도다 — 세면대 안 어안에서 올려다본 얼굴, 주황 림이 하단을 감싸고 좌우에 거울·튜브 조명, 얼굴 중앙 상단. s13 테이크의 얼굴 진입 완료 구간을 그대로 쓴다.
  ...
  ![](assets/thumbs/s01_start.jpg) ![](assets/thumbs/s13_end.jpg)

  왼쪽부터: 원본 s01 시작 · 원본 s13 끝 — 동일 구도(세면대 안 어안의 내려다보는 얼굴). 그래서 T05에서 오려도 된다. (**생성 입력 아님** — 편집 매핑 근거)
...
- **s02 ← T10(s20)**: `s02_start.jpg`는 왼쪽 전방 저위치 카메라가 본 "숙인 얼굴" 초근접(좌하향 3/4 얼굴, 초커 늘어짐, 민트 타일 보케). `s20_start.jpg`와 카메라 측·높이·얼굴 방향·배경이 일치하고 샷 크기만 한 단계 타이트 — T10 하강 초반 구간 + 펀치인 ~20%로 커버.
  ...
  ![](assets/thumbs/s02_start.jpg) ![](assets/thumbs/s20_start.jpg)

  왼쪽부터: 원본 s02 시작 · 원본 s20 시작 — 카메라 측·높이·얼굴 방향·배경 일치, 샷 크기만 한 단계 차이. 펀치인 ~20%로 커버. (**생성 입력 아님** — 편집 매핑 근거)

- **s03 → 이 팔에서 제외 (몽타주 실험 이관)**: 유사 후보 둘 다 프레임 대조로 기각. T09(s19)는 표준 렌즈·무릎 높이·배경이 카운터 전면(s03은 어안·정강이 높이·배경이 거울벽), T12(s23)는 피사체가 카운터 위 삭스 발(s03은 바닥에 선 허벅지~무릎)이고 양손으로 치맛단을 들어올리는 동작이 후반 어디에도 없다. 내용 결손이라 펀치인으로 구제 불가.
- **s04 ← T06(s15)**: `s04_start.jpg`와 `s15_start.jpg`는 같은 수전-기울임 순간의 같은 배치다 — 꽃 좌상단·크롬 수전 좌하단·숙인 옆얼굴이 중앙-우측. 차이는 샷 크기뿐 — T06 중반 + 펀치인 ~35%(얼굴+수전 영역)로 커버.
  ...
  ![](assets/thumbs/s04_start.jpg) ![](assets/thumbs/s15_start.jpg)

  왼쪽부터: 원본 s04 시작 · 원본 s15 시작 — 같은 수전-기울임 순간의 같은 배치, 차이는 샷 크기뿐. 펀치인 ~35%로 커버. (**생성 입력 아님** — 편집 매핑 근거)

- **s05 ← T16(s28)**: 콘티 정답지에 동일 구도 명시. 변기 옆 쓰러진 소녀 수직 탑뷰 — T16 초반 프레임 그대로.

  ![](assets/thumbs/s05_start.jpg) ![](assets/thumbs/s28_start.jpg)

  왼쪽부터: 원본 s05 시작 · 원본 s28 시작 — 콘티 정답지에 동일 구도로 명시된 쌍. (**생성 입력 아님** — 편집 매핑 근거)

- **s06 → 이 팔에서 제외 (몽타주 실험 이관)**: 바닥 높이에서 뒤따라가는 보행 구두 매크로. 후반의 발 관련 컷은 전부 정지 상태(s19 서 있는 다리, s23 카운터 위에 선 발)라 보행 인서트의 대응이 없다.
- **s07 → 이 팔에서 제외 (몽타주 실험 이관)**: 거울벽 앞 뒷모습 미디엄(정수리~허리, 실물 빨간 문 우측 1/3). 같은 축의 후반 컷 s10은 헤드숏 크기라 두 단계 차이 — 오려쓰기도 펀치아웃도 불가.
- **s08 → 이 팔에서 제외 (몽타주 실험 이관)**: 카운터 옆 전신 프로필로 파우치를 여는 컷. 파우치 개봉은 도입에서만 일어나는 사건이고, 후반 전신 컷(마스터)에선 인물이 프레임 높이 ~45%로 너무 작아 크기 대응이 안 된다.

제외 4컷(s03·s06·s07·s08)은 이 팔에서 제외하고 도입 몽타주 별개 실험([../2026-07-24_intro-montage/design.md](../2026-07-24_intro-montage/design.md))으로 이관한다(후반 테이크로 못 채우는 근거는 위 대조와 동일 — 오너 확정 2026-07-24).

### 3-3. 정적 인서트 2건 — 영상 생성 0콜 설계

s14(배수구 매크로 1.6s)와 s21(배관 인서트 1.5s)은 원본이 무인물 정적 컷이다. v1은 각각 4초 영상 테이크(T12·T18)로 생성했지만, BKM2는 **i2i 프레임 1장씩(F01·F02)을 편집에서 홀드 + 1~2% 디지털 푸시인**으로 처리한다. 근거: 원본의 움직임이 광택 미세 반짝임 수준이라 스틸+푸시로 재현 가능하고, 이는 편집자가 실제로 쓰는 표준 기법이므로 "사람이 쓸 수 있는 최고 연출" 정의 안이다. 영상 8초(~37크레딧) 절약. 오너가 미세 반짝임의 생동감을 원하면 v1 방식(4초 테이크 2개 추가)으로 되돌릴 수 있다 — 그 경우 총 20테이크·94초.
...
테이크 번호는 v2 신규 채번이다(v1 T번호와 무관). T01~T18 = 본편(커버 컷 s09 이후 시간순). 도입 전용 테이크는 없다 — 도입 플래시는 재사용 4컷 + 몽타주 실험 이관 4컷으로 처리한다(§3).

---

### T01 — 컷 s09 커버 · 6초 → `clips/arm-bkm2/T01.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s09_start.jpg) ![](assets/thumbs/s09_end.jpg)

왼쪽부터: 원본 s09 시작 · s09 끝 — QC 대조 기준 프레임 (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
왼쪽부터: 정본(신원) · 플레이트(공간) — 원본 픽셀은 입력에 안 들어간다(구도는 텍스트 지시로만 전달)

**1단계 · 시작 프레임 생성 (i2i)**

- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T01_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T01_end.jpg`
...
**3단계 · ▶ 영상 API 입력** — Seedance 2.0 힉스필드 레인 · task `i2v_se` · 6초 · 16:9

- 이미지 ①: `arm-bkm2/frames/T01_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T01_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T01.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s09 5.77~11.30 ← T01 0.2~5.7

- 원본 대응 프레임 (QC 대조): `../2026-07-23_full-copy-bundle/assets/conti/s09_start.jpg` · `s09_end.jpg`

<details><summary>jobs.bkm2.json 조각</summary>
...
### T02 — 컷 s10 커버 (+ s12 오버숄더 꼬리) · 4초 → `clips/arm-bkm2/T02.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s10_start.jpg) ![](assets/thumbs/s10_end.jpg) ![](assets/thumbs/s12_end.jpg)

왼쪽부터: 원본 s10 시작 · s10 끝 — QC 대조 기준 · s12 끝(오버숄더 꼬리 — 이 테이크 말미로 커버) (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T02_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T02_end.jpg`
...
**3단계 · ▶ 영상 API 입력** — Seedance 2.0 힉스필드 레인 · task `i2v_se` · 4초 · 16:9

- 이미지 ①: `arm-bkm2/frames/T02_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T02_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T02.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s10 11.30~14.73 ← T02 0.2~3.6 · s12 꼬리(오버숄더 시선 하강) ← T02 3.0~4.0
...
### T03 — 컷 s11 · s17 커버 (마스터 α+β 통합) · 8초 → `clips/arm-bkm2/T03.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s11_start.jpg) ![](assets/thumbs/s17_start.jpg) ![](assets/thumbs/s17_end.jpg)

왼쪽부터: 원본 s11 시작 · s17 시작 · s17 끝 — QC 대조 기준 프레임 (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T03_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T03_end.jpg`
...
**3단계 · ▶ 영상 API 입력** — Seedance 2.0 힉스필드 레인 · task `i2v_se` · 8초 · 16:9

- 이미지 ①: `arm-bkm2/frames/T03_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T03_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T03.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s11 14.73~16.17 ← T03 0.3~1.7 (마스터 정지 구간) · s17 29.50~33.03 ← T03 4.3~7.8
...
### T04 — 컷 s12 본체 커버 · 4초 → `clips/arm-bkm2/T04.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s12_start.jpg)

원본 s12 시작 — QC 대조 기준 (s12 끝의 오버숄더 꼬리는 T02 담당) (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T04_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T04_end.jpg`
...
- 이미지 ①: `arm-bkm2/frames/T04_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T04_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T04.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s12 본체 16.17~19.47 ← T04 0.4~2.7 (꼬리는 T02 담당)
...
### T05 — 컷 s13 커버 (+ 도입 s01 · s23 꼬리 재사용) · 6초 → `clips/arm-bkm2/T05.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s13_start.jpg) ![](assets/thumbs/s13_end.jpg) ![](assets/thumbs/s01_start.jpg) ![](assets/thumbs/s23_end.jpg)

왼쪽부터: 원본 s13 시작 · s13 끝 — QC 대조 기준 · s01 시작(도입 재사용 목표) · s23 끝(꼬리 대체 목표) (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
왼쪽부터: 정본(신원 — 인물이 새로 등장하는 2단계에서만 투입) · 플레이트(공간 — 1단계 참조) — 원본 픽셀은 입력에 안 들어간다(구도는 텍스트 지시로만 전달)

**1단계 · 시작 프레임 생성 (i2i)**

- 넣는 것: 플레이트 + 아래 프롬프트 (1단계는 무인물 — 빈 림 상태로 시작) → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T05_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 정본(얼굴 신원 앵커 — 인물이 새로 등장하는 유일한 테이크) + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T05_end.jpg`
...
- 이미지 ①: `arm-bkm2/frames/T05_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T05_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T05.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s13 19.47~23.10 ← T05 0.0~3.6 · s01 0.00~0.37 ← T05 4.6~5.0 (도입 재사용) · s23 꼬리 ← T05 4.4~5.9 (정지 응시 구간 대체)
...
### T06 — 컷 s15 커버 (+ 도입 s04 재사용) · 4초 → `clips/arm-bkm2/T06.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s15_start.jpg) ![](assets/thumbs/s15_end.jpg) ![](assets/thumbs/s04_start.jpg)

왼쪽부터: 원본 s15 시작 · s15 끝 — QC 대조 기준 · s04 시작(도입 재사용 목표, 펀치인 ~35%) (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T06_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T06_end.jpg`
...
- 이미지 ①: `arm-bkm2/frames/T06_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T06_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T06.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s15 24.67~27.73 ← T06 0.4~3.5 · s04 1.20~1.50 ← T06 2.2~2.5 + 펀치인 ~35% (도입 재사용)
...
### T07 — 컷 s16 커버 · 4초 → `clips/arm-bkm2/T07.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s16_start.jpg) ![](assets/thumbs/s16_end.jpg)

왼쪽부터: 원본 s16 시작 · s16 끝 — QC 대조 기준 프레임 (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T07_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T07_end.jpg`
...
- 이미지 ①: `arm-bkm2/frames/T07_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T07_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T07.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s16 27.73~29.50 ← T07 1.0~2.8
...
### T08 — 컷 s18 커버 · 4초 → `clips/arm-bkm2/T08.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s18_start.jpg) ![](assets/thumbs/s18_end.jpg)

왼쪽부터: 원본 s18 시작 · s18 끝 — QC 대조 기준 프레임 (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T08_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T08_end.jpg`
...
- 이미지 ①: `arm-bkm2/frames/T08_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T08_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T08.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s18 33.03~34.07 ← T08 1.5~2.5
...
### T09 — 컷 s19 커버 · 4초 → `clips/arm-bkm2/T09.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s19_start.jpg) ![](assets/thumbs/s19_end.jpg)

왼쪽부터: 원본 s19 시작 · s19 끝 — QC 대조 기준 프레임 (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T09_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T09_end.jpg`
...
- 이미지 ①: `arm-bkm2/frames/T09_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T09_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T09.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s19 34.07~35.47 ← T09 1.2~2.6
...
### T10 — 컷 s20 커버 (+ 도입 s02 재사용) · 4초 → `clips/arm-bkm2/T10.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s20_start.jpg) ![](assets/thumbs/s20_end.jpg) ![](assets/thumbs/s02_start.jpg)

왼쪽부터: 원본 s20 시작 · s20 끝 — QC 대조 기준 · s02 시작(도입 재사용 목표, 펀치인 ~20%) (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T10_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T10_end.jpg`
...
- 이미지 ①: `arm-bkm2/frames/T10_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T10_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T10.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s20 35.47~37.17 ← T10 2.2~3.9 · s02 0.37~0.63 ← T10 0.3~0.6 + 펀치인 ~20% (도입 재사용)
...
### T11 — 컷 s22 커버 · 4초 · 2인(bible_two) → `clips/arm-bkm2/T11.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s22_start.jpg) ![](assets/thumbs/s22_end.jpg)

왼쪽부터: 원본 s22 시작 · s22 끝 — QC 대조 기준 프레임 (카운터 위 두 발 = 도플갱어 복선) (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
정본(두 동일 인물의 신원 겸용) — 플레이트 미사용, 원본 픽셀은 입력에 안 들어간다(구도는 텍스트 지시로만 전달)

**1단계 · 시작 프레임 생성 (i2i)**

- 넣는 것: 정본 + 아래 프롬프트 (2인 테이크 — 정본이 두 인물 신원 겸용, 플레이트 없음) → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T11_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T11_end.jpg`
...
- 이미지 ①: `arm-bkm2/frames/T11_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T11_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T11.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s22 38.67~41.47 ← T11 0.6~3.4
...
### T12 — 컷 s23 본체 커버 · 6초 → `clips/arm-bkm2/T12.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s23_start.jpg)

원본 s23 시작 — QC 대조 기준 (드레인 POV 꼬리인 s23 끝은 T05가 대체 담당) (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T12_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T12_end.jpg`
...
- 이미지 ①: `arm-bkm2/frames/T12_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T12_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T12.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s23 본체 41.47~46.97 ← T12 0.3~4.3 (꼬리는 T05 담당)
...
### T13 — 컷 s25 커버 · 4초 · 2인(bible_two) → `clips/arm-bkm2/T13.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s25_start.jpg) ![](assets/thumbs/s25_end.jpg)

왼쪽부터: 원본 s25 시작 · s25 끝 — QC 대조 기준 프레임 (끌기는 s25에서 이미 시작돼 있다) (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
- 넣는 것: 정본 + 아래 프롬프트 (2인 테이크 — 정본이 두 인물 신원 겸용, 플레이트 없음) → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T13_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T13_end.jpg`
...
- 이미지 ①: `arm-bkm2/frames/T13_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T13_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T13.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s25 48.77~50.77 ← T13 1.0~3.0
...
### T14 — 컷 s26 커버 · 5초 · 2인(bible_two) · QC 반례 기준 → `clips/arm-bkm2/T14.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s26_start.jpg) ![](assets/thumbs/s26_end.jpg)

왼쪽부터: 원본 s26 시작 · s26 끝 — QC 대조 기준 프레임 (두 인물이 좌측 1/3에 작게 — v1이 실패한 바로 그 구도) (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
왼쪽부터: 정본(두 동일 인물의 신원 겸용) · 플레이트(공간 — 마스터 구도 앵커) — 원본 픽셀은 입력에 안 들어간다(구도는 텍스트 지시로만 전달)

**1단계 · 시작 프레임 생성 (i2i)**

- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 (마스터 구도라 플레이트 앵커 포함) → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T14_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T14_end.jpg`
...
**3단계 · ▶ 영상 API 입력** — Seedance 2.0 힉스필드 레인 · task `i2v_se` · 5초 · 16:9

- 이미지 ①: `arm-bkm2/frames/T14_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T14_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T14.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s26 50.77~53.50 ← T14 1.2~3.9
...
### T15 — 컷 s27 커버 · 4초 → `clips/arm-bkm2/T15.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s27_start.jpg) ![](assets/thumbs/s27_end.jpg)

왼쪽부터: 원본 s27 시작 · s27 끝 — QC 대조 기준 프레임 (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T15_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T15_end.jpg`
...
- 이미지 ①: `arm-bkm2/frames/T15_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T15_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T15.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s27 53.50~55.90 ← T15 0.8~3.2
...
### T16 — 컷 s28 커버 (+ 도입 s05 재사용) · 6초 → `clips/arm-bkm2/T16.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s28_start.jpg) ![](assets/thumbs/s28_end.jpg) ![](assets/thumbs/s05_start.jpg)

왼쪽부터: 원본 s28 시작 · s28 끝 — QC 대조 기준 · s05 시작(도입 재사용 목표 — 콘티 명시 동일 구도) (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T16_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T16_end.jpg`
...
- 이미지 ①: `arm-bkm2/frames/T16_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T16_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T16.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s28 55.90~60.90 ← T16 0.8~5.8 · s05 1.50~1.63 ← T16 0.3~0.4 (도입 재사용, 콘티 명시 s05=s28)
...
### T17 — 컷 s29 커버 · 4초 → `clips/arm-bkm2/T17.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s29_start.jpg) ![](assets/thumbs/s29_end.jpg)

왼쪽부터: 원본 s29 시작 · s29 끝 — QC 대조 기준 프레임 (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T17_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T17_end.jpg`
...
- 이미지 ①: `arm-bkm2/frames/T17_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T17_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T17.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s29 60.90~61.80 ← T17 1.4~2.3
...
### T18 — 컷 s30 커버 · 5초 → `clips/arm-bkm2/T18.mp4`

**원본 목표 (이 테이크가 흉내내야 하는 화면):**

![](assets/thumbs/s30_start.jpg)

원본 s30 시작 — QC 대조 기준 (s30 끝은 암전이라 대조 제외) (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
- 넣는 것: 정본 + 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/T18_start.jpg`
...
- 넣는 것: 1단계 시작 프레임 + 아래 프롬프트
- 나오는 것: `assets/arm-bkm2/frames/T18_end.jpg`
...
- 이미지 ①: `arm-bkm2/frames/T18_start.jpg` (시작)
- 이미지 ②: `arm-bkm2/frames/T18_end.jpg` (끝)
- 텍스트: 아래 코드블록 전문 (공통 계약 §4-0 포함, 무수정)
- 산출: `clips/arm-bkm2/T18.mp4`
...
**편집에서 이 테이크가 커버하는 컷**: s30 61.80~65.90 ← T18 0.5~4.6 (65.9 이후 암전은 편집)
...
### F01 — 컷 s14 커버 (배수구 매크로 · 스틸) · 영상 호출 없음 — 스틸 1장 + 편집 홀드 1.6s → `assets/arm-bkm2/frames/F01.jpg`

**원본 목표 (이 스틸이 흉내내야 하는 화면):**

![](assets/thumbs/s14_start.jpg)

원본 s14 시작 — QC 대조 기준 프레임 (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
플레이트(공간)만 — 무인물 스틸이라 정본은 안 들어가고, 원본 픽셀도 입력에 안 들어간다

**1단계 · 스틸 생성 (i2i — 이 1장이 최종 산출, 2단계·3단계 없음)**

- 넣는 것: 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/F01.jpg`
...
**영상 호출 없음 — 스틸 1장 + 편집 홀드** (§3-3)

**편집에서 이 스틸이 커버하는 컷**: s14 23.10~24.67 (홀드 1.6s + 푸시인 1~2%)

### F02 — 컷 s21 커버 (배관 인서트 · 스틸) · 영상 호출 없음 — 스틸 1장 + 편집 홀드 1.5s → `assets/arm-bkm2/frames/F02.jpg`

**원본 목표 (이 스틸이 흉내내야 하는 화면):**

![](assets/thumbs/s21_start.jpg)

원본 s21 시작 — QC 대조 기준 프레임 (**생성 입력 아님** — 생성에 들어가는 건 아래 i2i 재료뿐)
...
- 넣는 것: 플레이트 + 아래 프롬프트 → `openai/gpt-image-2/edit`
- 나오는 것: `assets/arm-bkm2/frames/F02.jpg`
...
**편집에서 이 스틸이 커버하는 컷**: s21 37.17~38.67 (홀드 1.5s + 푸시인 1~2%)
...
- **산출**: `assets/arm-bkm2/frames/T{01..18}_{start,end}.jpg` 36장 + `F01.jpg`·`F02.jpg` 2장 = i2i 38콜. 완료 후 `jobs.bkm2.json`(18잡, §4의 조각과 동일 내용) 생성.
- **동작 승계**: fal `openai/gpt-image-2/edit` · resume 상태 파일 `assets/bkm2_state.json` · 업로드 캐시 · 동시 3 · 실패 2회 재시도 후 기록 · 콜 상한 60(재시도 여유 포함) · `PHASE=dry` 드라이런.
...
|---|---|---|
| 영상 18테이크 | 86초 | ~396 |
| QC 불합격 재생성 여유 15% | ~13초 | ~59 |
| **영상 소계** | | **~455** |
| i2i 프레임 (fal, 별도 과금) | 38콜 (+재시도 상한 60) | — |
| 정적 인서트 s14·s21 | 영상 0콜 (스틸 홀드) | 0 |
| s24 블랙 | 0콜 | 0 |

v1 대비: 테이크 27→18(−9), 생성 초수 108→86(−22초). 도입 4컷 재사용 + 도입 전용 4컷의 몽타주 실험 이관(오너 확정 2026-07-24) + 정적 인서트 2건 스틸 전환으로 줄었다. 상위 설계(design.md §6)의 "BKM2 ~100s" 개산보다 낮다 — 차이 −14초는 도입 전용분의 몽타주 실험 이관 때문이다.
...
- 2026-07-24 개정: 오너 확정 — 도입 몽타주를 별개 실험으로 분리, 도입 전용 테이크 4건을 이 팔에서 제거(해당 컷은 생성 없음, 몽타주 실험 산출로 채움). 테이크 22→18 · 생성 초수 102→86 · i2i 콜 46→38 · 영상 소계 ~540→~455크레딧. 원본 프레임 임베드 캡션에 "생성 입력 아님" 명시(오너 오독 교정).
