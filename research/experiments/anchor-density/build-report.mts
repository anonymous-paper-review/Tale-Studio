...
for (const t of [0, 1, 2, 3, 4, 5, 6, 7]) {
  const f = join(DIR, 'inputs', `anchor_${t}.jpg`)
...
const html = `<title>앵커를 몇 장 주어야 하는가</title>
<style>${css}
...
<header class="page">
  ...
  <h1>앵커를 몇 장 주어야, 어디부터가 과잉인가</h1>
  <p class="standfirst">시작 그림과 끝 그림 사이를 이미지 생성기로 1초마다 채우고, 영상 생성기에 <b>1장 · 2장 · 3장 · 4장 · 5장 · 8장</b>으로 나눠 넣었습니다. 각 조건마다 <b>시작 그림의 깊이 지도를 함께 넣은 짝</b>을 두어 두 배로 돌렸습니다. <b>판정은 하지 않았습니다.</b></p>
...
<section class="block">
  ...
  <details>
    ...
    <div class="body">
      ...
      <pre class="prompt">${esc(koFull(done[0]?.anchor_times ?? [0], Boolean(done[0]?.depth), 'anchor-density'))}</pre>
  ...
  <details>
    ...
    <div class="body">
      <p class="note"><b>한국어</b></p>
      <pre class="prompt">${esc(koFull(maxJob?.anchor_times ?? [0], true, 'anchor-density'))}</pre>
      <p class="note"><b>실제로 보낸 원문(영어)</b></p>
