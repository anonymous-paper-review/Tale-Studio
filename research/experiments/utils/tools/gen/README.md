...
```bash
# fal — 키는 이 레포 .env.local(FAL_KEY)에 있음. 스탠드얼론 mjs는 .env를 자동으로 안 읽으므로
#        실행 시 node --env-file=.env.local 로 주입한다 (아래 참고).
...
레포 루트에서 실행. fal 레인을 쓰면 `--env-file=.env.local` 로 FAL_KEY를 주입한다.
...
> higgsfield 단독(`--mode higgsfield`)이면 FAL_KEY가 필요 없어 `--env-file` 생략 가능.
