...
## 6b. previz ①+③ A/B + 제품 이관 (2026-08-07 오후)

방향 전환(사용자): viz 프롬프트 보정은 증상 치료 — 뿌리는 previz 정보량. 러프는 viz 리페인트의
주 레퍼런스이자 영상의 [START,END] 키프레임 원천이라, 러프에 없는 의도는 하류가 지어낸다.

**previz A/B (프로덕트 미반영 하네스, 클론 f79546f6, 그리드 2쌍)**: 동일 spec·모델·템플릿에
treatment 프롬프트만 강화 —
- ① DIRECTION 행(유일 텍스트 허용 행)에 손글씨 기술 라벨: KEY(조명 방향+질)/사이즈·렌즈·앵글·
  심도/FOCUS(초점 서술)/색온도(WARM 3500K — 흑백 previz 의 유일한 색 통로).
- ③ START·END 스케치 자체에 빛·초점: 방향성 연필 해칭(그림자 반대 방향 명시, hard=crisp/
  soft=feathered), 초점 피사체는 최밀 디테일 + shallow 면 배경 느슨하게.

결과(육안, 2쌍 4장): 라벨 전부 정확히 렌더, 구덩이 내부 방향성 어둠·광축 그림자 등 ③ 도
스케치에 실림. 목각 캐논·구도 유지. 한계: 얼굴 초점은 라벨로만(목각 무안면), 해칭으로 러프가
덜 미니멀해짐(의도된 트레이드오프), viz 갭 실축소는 미측정.

**제품 이관(사용자 결정 — 실페이지 테스트로 평가)**: 하네스 enrich 로직을 buildRoughGridCell 에
통합(rich staticSpec 경로 전용, fallback 불변). 그리드·스트립(단일 재생성) 공용. 프롬프트 말미
