...
> 새벽 코인세탁방 이야기로 60초짜리 영상 딱 하나를 만들었다. 결과물: [final_60s.mp4](assets/e9d-proto/final_60s.mp4) (60.6초).
>
...
**영상: [final_60s.mp4](assets/e9d-proto/final_60s.mp4)** (60.6초 · 1920×1080 · 무음)

**1초 간격 필름스트립(전체 흐름 한눈에):**

![필름스트립](assets/e9d-proto/filmstrip.jpg)
...
`research/writer/experiments/tools/e9dp_generate.mjs` — `src/lib/writer/llm/fal.ts`의 fal 입력 조립을
복제(모델 id·입력 스키마 동일).
...
`research/writer/experiments/tools/e9dp_assemble.mjs` — 각 클립을 `render_seconds`로 트리밍 +
...
### 산출물 (`research/writer/experiments/results/assets/e9d-proto/`)
...
# 2) 샷플랜 조립 (결정론)
E9DP_RUN=e9dp1 node research/writer/experiments/tools/e9dp_build_shotplan.mjs
# 3) 이미지·영상 생성 (유료: fal). PHASE=images|videos|all, gen_state.json으로 resume
set -a && source .env.local && set +a
PHASE=all node research/writer/experiments/tools/e9dp_generate.mjs
# 4) 트리밍·concat·필름스트립 (ffmpeg)
node research/writer/experiments/tools/e9dp_assemble.mjs
```
...
- 관련: 레퍼런스 해석 `../references/2026-07-22-girls-in-mirror-continuity.md`, 모델 티어 근거
  `E9c-model-tier.md`(B' 채용), 실험 카드 `../2026-07-21-experiment-plan.md` Phase 5.
