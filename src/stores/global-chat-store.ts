...
interface GlobalChatState {
  ...
  sendMessage: (
    ...
    opts?: { consentedHandoff?: boolean; stageOverride?: StageId },
  ) => Promise<void>
...
export const useGlobalChatStore = create<GlobalChatState>((set, get) => ({
  ...
  sendMessage: async (content, attachments, opts) => {
    ...
    if (pendingProposal && pendingProposal.stage === stage && isCancellationUtterance(trimmed)) {
    ...
    const handoffSpec = dialogueTarget ? null : matchHandoffIntent(trimmed, stage)
    if (handoffSpec) {
        ...
        for (const costUpdate of costUpdates) {
          ...
          const proposal = costUpdate.type === 'regenerateCharacter'
            ...
            : createPendingProposal({
                ...
                payload: { locationId: costUpdate.locationId, appearanceKey: costUpdate.appearanceKey },
              })
  ...
  approvePendingProposal: async (id) => {
    ...
    try {
      ...
      if (proposal.kind === 'producerSourcePatch') {
        ...
        if (typeof appearanceKey === 'string' && appearanceKey !== 'default') {
          ...
          await useArtistStore.getState().generateWorldShot(locationId, 'wideShot', undefined, 'chat', undefined, { appearanceKey, traceId: traceId ?? undefined, onJob: observeGeneration })
        } else {
    ...
    for (const item of items) {
      ...
      const run = (async () => {
        ...
        if (succeeded) {
          ...
          if (!receipt && item.stage === 'artist') speak(translate(contentLocale(), '{target}: {status}{reason}', {
    ...
    return isCurrentSession() && allSucceeded && results.every(Boolean)
  },
