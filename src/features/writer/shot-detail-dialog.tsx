...
//
// 편집은 로컬 draft 로만 하고(실시간 저장 없음), "저장" 버튼을 눌러야 store→DB 에 커밋한다.
//   (예전엔 keystroke 마다 updateShot 디바운스 저장 → 편집 도중 서버 반영/재생성이 튀는 문제. 2026-07-11)
// 재생성(방향 칩)은 현재 draft 를 먼저 저장·플러시한 뒤 발사해 최신 스토리로 그린다.
...
export function ShotDetailDialog({
  ...
  if (shotId !== draftForShotId) {
    ...
    setDraft(
      shot
        ...
        : { shotType: 'MS', durationSeconds: 5, actionDescription: '' },
    )
  }
  ...
  const handleRegenerate = async (styleHints?: string[]) => {
    // draft 를 먼저 저장(디바운스)하고 플러시를 기다린 뒤 재생성 — 최신 스토리로 그리게.
    commitDraft()
    setFlushing(true)
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div>
                        <Select
                          ...
                          onValueChange={(v) =>
                            setDraft((d) => ({ ...d, shotType: v as ShotType }))
                          }
                        >
                    ...
                    <TooltipContent>
                      {SHOT_TYPE_DESCRIPTIONS[draft.shotType] ?? draft.shotType}
                    </TooltipContent>
                  </Tooltip>
                </div>
                <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <HoverBeam>
                  <Textarea
                    ...
                    rows={4}
                    onChange={(e) =>
                      setDraft((d) => ({ ...d, actionDescription: e.target.value }))
                    }
