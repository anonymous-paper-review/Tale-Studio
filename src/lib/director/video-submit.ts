...
function decodeReceipt(receipt: unknown, storedBatchRecovery = false): RecoveryReceiptPayload {
...
async function readVideoReplay(
  projectId: string,
  writerShotId: string,
  videoClipId: string | null | undefined,
  ...
  const { data: existingReplay, error } = videoClipId
    ? await query.eq('video_clip_id', videoClipId).maybeSingle()
    : await query.contains('target', { retakeMode: 'new_take' }).maybeSingle()
  if (error) throw error
  const target = existingReplay && typeof existingReplay.target === 'object' && existingReplay.target !== null && !Array.isArray(existingReplay.target)
    ? existingReplay.target as Record<string, unknown>
    : null
  const exactReplay = Boolean(existingReplay) && (
    videoClipId
      ? target?.retakeMode === 'regeneration' && target.writerShotId === writerShotId && target.videoClipId === videoClipId
      : target?.retakeMode === 'new_take' && target.writerShotId === writerShotId
  )
  return { existingReplay, exactReplay }
}

export interface PreparedDirectorVideoSubmission {
  ownerId: string
  projectId: string
  workspaceId: string
  writerShotId: string
  ...
  standalone: boolean
  standaloneConfig: StandaloneVideoConfig | null
  modelKey: VideoModelKey
  takeAmount: number
  provider: VideoProvider
  idempotencyKey: string
  traceId: string | null
  jobActor: 'chat' | 'ui'
  inputSnapshot: Record<string, Json | undefined>
...
export interface PreparedVideoSubmitOptions {
  reserve?: (input: Parameters<typeof reserveDirectorVideoTake>[0]) => ReturnType<typeof reserveDirectorVideoTake>
  recoveryReceipt?: string
  /** 서버에 저장된 같은 묶음의 증표만 만료 후에도 서명 검증하여 연결한다. 외부 body로 받지 않는다. */
  batchRecoveryId?: string
}

export async function submitDirectorVideoRequest(req: Request): Promise<Response> {
  const prepared = await prepareVideoRequest(req, null)
  if (prepared instanceof Response) return prepared
  return submitPreparedVideo(prepared, {}, true)
}

/** #batch-resume: 서버가 확정한 신원으로 입력만 고정한다. 예약과 과금은 하지 않는다. */
export function prepareDirectorVideoSubmission(req: Request, trustedUserId: string) {
  return prepareVideoRequest(req, { userId: trustedUserId })
}

/** 외부 body로는 호출하지 않는다. 준비해 저장한 입력만 이 진입점으로 제출한다. */
export function submitPreparedDirectorVideo(
  prepared: PreparedDirectorVideoSubmission,
  options: PreparedVideoSubmitOptions = {},
): Promise<Response> {
  return submitPreparedVideo(prepared, options, false)
}

async function prepareVideoRequest(
  req: Request,
  trustedCaller: { userId: string } | null,
): Promise<PreparedDirectorVideoSubmission | Response> {
  const demoBlocked = demoWriteBlock(req)
  if (demoBlocked) return demoBlocked
  let projectId = ''
  try {
    const identity = await resolveSubmitIdentity(trustedCaller === null
      ? { getUser }
      : { getUser, onBehalfOfUserId: trustedCaller.userId })
    ...
    const { existingReplay, exactReplay } = await readVideoReplay(projectId, writerShotId, videoClipId, idempotencyKey)
    ...
    if (!exactReplay && trustedCaller === null) {
    ...
    return JSON.parse(JSON.stringify({
      ...
      jobActor,
      inputSnapshot,
      takeLabel: normalizedNewTakeMetadata.take_label,
      override: normalizedNewTakeMetadata.override,
      canvasPosition: normalizedNewTakeMetadata.canvas_position,
      recoveryReceipt: recoveryReceipt ?? null,
    })) as PreparedDirectorVideoSubmission
  } catch (err) {
    ...
    console.error('[director/generate-video] preparation failed:', error)
    return NextResponse.json({ error }, { status: /clip already has a queued attempt|idempotency mismatch/i.test(error) ? 409 : 500 })
  }
}

async function submitPreparedVideo(
  prepared: PreparedDirectorVideoSubmission,
  options: PreparedVideoSubmitOptions,
  checksPerformed: boolean,
): Promise<Response> {
  let reservation: Awaited<ReturnType<typeof reserveDirectorVideoTake>> | null = null
  ...
  const normalizedNewTakeMetadata = {
    ...
    canvas_position: prepared.canvasPosition,
  }
  try {
    if (!checksPerformed) {
      ...
      if (recoveryReceipt && !exactReplay) {
        return NextResponse.json({ error: 'Invalid request: recoveryReceipt can only recover an existing reservation' }, { status: 409 })
      }
      if (!exactReplay) {
        const budget = await checkProjectVideoBudget(projectId, user.id)
        if (!budget.ok) return videoBudgetRejectionResponse(budget, { projectId, kind: 'shot_video', userId: user.id })
    ...
    if (!reservation.replayed) {
      const holdAmount = prepared.takeAmount
    ...
    if (recoveryReceipt) {
      if (options.batchRecoveryId && (!options.reserve || reservedJob.batch_id !== options.batchRecoveryId)) {
        throw new RecoveryInputError(409, 'Recovery receipt does not belong to the stored batch')
      }
      const receipt = decodeReceipt(recoveryReceipt, Boolean(options.batchRecoveryId))
      if (
