...
import { dialogueLanguageClause, outputLanguageClause, speechRateGuideForDialogue, type DialogueLanguage } from '@/lib/writer/pipeline/util/output-language';
import type {
  Genre,
  Characters,
  Scenes,
...
function buildVoiceSystem(outputLocale?: AppLocale, dialogueLanguage?: DialogueLanguage): string {
...
4. 집단 인물(예: 병사들)은 집단의 발화 방식(구호/무전/침묵)으로 정의한다.${outputLanguageClause(outputLocale)}${dialogueLanguageClause(dialogueLanguage)}`;
}
...
[출력 형식 - JSON]
{
  ...
  const raw = await generateJson<{ profiles?: VoiceProfile[] } | VoiceProfile[]>(userPrompt, axisConfig, {
    systemInstruction: buildVoiceSystem(outputLocale, dialogueLanguage),
    temperature: 0.7,
  });
...
// #i18n-s5: 발화 속도 기준 — 대사 언어 설정이 있으면 그 언어(#dialogue-language), 없으면 출력 언어(종전).
function buildDialogueSystem(outputLocale?: AppLocale, dialogueLanguage?: DialogueLanguage): string {
...
3. 분량 = 샷 길이: ${speechRateGuideForDialogue(dialogueLanguage, outputLocale)}, 샷 duration 안에 실제로 말할 수 있는 분량만.
   (4초 샷에 두 문장 ❌)
...
C. 명대사 제조 금지 강화: 주제를 요약·선언하는 문장("진실은 ~하지 않아", "이제 나는 ~다") 금지.
   콜백은 [이미 나온 주요 대사]의 짧은 재사용만 허용 — 새로운 선언문을 만들지 마라.${outputLanguageClause(outputLocale)}${dialogueLanguageClause(dialogueLanguage)}`;
}
...
[출력 형식 - JSON]
{
  ...
  const raw = await generateJson<SceneDialogueResponse>(userPrompt, axisConfig, {
    systemInstruction: buildDialogueSystem(outputLocale, dialogueLanguage),
    temperature: 0.7,
  });
...
export async function runDialogue(
  ...
  opts?: {
    ...
    outputLocale?: AppLocale;
    /** #dialogue-language: 발화 언어 설정 — line·narration 만 이 언어로(출력 언어와 독립). */
    dialogueLanguage?: DialogueLanguage;
  },
  ...
  const dialogueLanguage = opts?.dialogueLanguage;
  ...
  const profiles =
    resume?.profiles ?? (await runVoiceProfiles(story, characters, genre, logger, axisConfig, outputLocale, dialogueLanguage));
  ...
  const attemptScene = async (
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        return await dialogueForScene(story, scene, shots, characters, profiles, mem, logger, axisConfig, outputLocale, dialogueLanguage);
      } catch (e) {
        console.warn(
