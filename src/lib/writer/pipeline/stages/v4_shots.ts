...
- 카메라 큰 무브 + 캐릭터 큰 액션 + 환경 변화 동시 금지${process.env.WRITER_ENERGY_EXCEPTION === '0' ? '' : `
  — 단(#energy-exception, 오너 결정 2026-09-07: 전후 비교 뒤 켬) motivation=energy 인 액션 비트에서는 카메라 큰 무브와 인물 큰 액션을 함께 쓴다. 환경 변화만 따로 둔다.`}
