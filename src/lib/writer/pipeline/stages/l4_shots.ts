...
export async function runL4Shots(
  ...
  await logger.markStage('L4_shots', 'started', { compact_mode: compactMode, decoupage_driven: decoupage !== null });
  ...
  for (const scene of s3.scenes) {
    ...
    const sceneShots = await generateL4ForScene(scene, plan, sceneDec, s0, s2, l1, l2, logger, axisConfig);
...
async function generateL4ForScene(
  ...
  sceneDec: DecoupageShot[] | null,  // 감독 데쿠파주 샷 목록. null이면 L4가 자체 결정 (legacy)
  s0: S0Genre,
  s2: S2Block,
  l1: L1Style,
  l2: L2Design,
