...
[출력 언어 — 강제]
화면에 노출되는 모든 자유서술 텍스트 필드(이름·표시명·설명·요약·씨/샷 서술·대사·질문 등)는 스토리 입력의 언어와 무관하게 예외 없이 ${lang}로 작성한다. 슬러그/id 는 규칙대로 snake_case 를 유지한다. 영어 문장이나 영문 텍스트를 쓸 때는 첫 글자를 반드시 대문자로 시작한다. 스키마나 다른 규칙이 특정 필드에 영어를 명시적으로 요구하면(생성기용 프롬프트 등) 그 규칙이 우선한다.`;
}
