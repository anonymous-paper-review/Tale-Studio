...
// 전역(프로젝트 1:1) 토큰만 기록한다 — v0 VisualIdentity + v2 WorldVisual 전역부.
//
...
import type { VisualIdentity, WorldVisual } from '@/lib/writer/types/pipeline'

const UUID_RE =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i

export interface DesignTokens {
  l0: VisualIdentity['format']        // = 옛 RenderFormat (DB JSONB 키는 l0 유지)
  l1: VisualIdentity['style']         // = 옛 ArtDirection
  palette: WorldVisual['global_palette']
  color_meaning: WorldVisual['color_meaning']
  vfx_approach: string
}
...
export async function persistDesignTokens(
  ...
  const design_tokens: DesignTokens = {
    l0: visualIdentity.format,
    l1: visualIdentity.style,
    palette: worldVisual.global_palette,
    color_meaning: worldVisual.color_meaning,
    vfx_approach: worldVisual.vfx_approach,
  }
