...
export function buildRoughGridCell(input: RoughStoryboardPromptInput, shotId: string): RoughGridCell {
  ...
  const motion = labels.length
    ? `${motionBase}. In addition to the motion arrows, letter these technical labels as a compact caption block along the bottom edge INSIDE the DIRECTION panel (small handwriting, shrink so EVERY label fits fully inside the panel, never cover the drawing's subject, never write outside the panel or between panels): ${labels.map((l) => `"${l}"`).join(', ')}`
    : motionBase
