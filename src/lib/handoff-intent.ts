...
/** A media-generation prohibition does not cancel a separately requested existing-screen move. */
export function withoutMediaGenerationProhibition(text: string): string {
  return text
    .replace(/(?:이미지|영상|동영상)(?:와\s*(?:이미지|영상))?\s*생성(?:은|을|는)?\s*(?:하지\s*마(?:세요)?|하지\s*말아\s*줘)/g, '') // i18n-ok: 생성 금지의 범위를 이동 요청과 분리.
    .replace(/\b(?:do not|don't)\s+generate\s+(?:any\s+)?(?:images?|videos?)\b/gi, '')
}

// 이 함수는 대화 전체를 해석하는 에이전트가 아니라 즉시 이동하는 빠른 경로다.
// 확인된 부정·질문·인용 표현은 일반 채팅에 남긴다. 모든 자연어의 의도 판정을 보장하지 않는다.
export function hasNonRequestContext(text: string): boolean {
  if (/[?？]/.test(text)) return true
  const compact = normalize(text)
  if (/조회만|상태만|확인만|(?:할|갈|넘길)수있(?:는지|나요|을까)/.test(compact)) return true // i18n-ok: 상태 질문을 실제 이동 요청과 구분.
...
export function matchHandoffIntent(text: string, stage: StageId): HandoffSpec | null {
  ...
  if (!spec) return null
  const request = omitQuotedRequests(['writer', 'artist'].includes(spec.to) ? withoutMediaGenerationProhibition(text) : text)
