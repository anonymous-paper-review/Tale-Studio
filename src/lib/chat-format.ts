...
- 구분선(---, ㅡㅡㅡ, ——— 등)을 쓰지 않는다 — UI가 수평선을 렌더하지 않아 생 기호로 노출된다. 문단 구분은 빈 줄만 쓴다.
- 사용자에게 보이는 문장에서 내부 id(sh_02_07, sc_04, char_2, char_new_xxx, loc_xx 같은 것)와
  대괄호 마커([p3] 같은 것)를 절대 쓰지 않는다 — 샷은 "씬2 · 샷7"(영어 응답이면 "Scene 2 · Shot 7"),
  씬은 "씬4", 인물·장소는 표시 이름으로 부른다. 내부 id 는 JSON 필드(updates 등) 안에서만 쓴다.
  (스크립트 라인 참조 [L3] 은 실제 기능이라 예외.)
</출력 형식>`
