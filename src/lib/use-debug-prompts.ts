...
export function useDebugPrompts(projectId: string | null | undefined): boolean {
  ...
  // projectId 변경 시 리셋은 렌더 중 조정 패턴으로 — effect 안 동기 setState 는 연쇄 렌더라
  //   lint 가 막는다(react-hooks/set-state-in-effect). 캐시가 있으면 그 값으로 즉시 복원.
  const [prevPid, setPrevPid] = useState<string | null>(projectId ?? null)
  if ((projectId ?? null) !== prevPid) {
    setPrevPid(projectId ?? null)
    setEnabled(projectId ? (flagCache.get(projectId) ?? false) : false)
  }
