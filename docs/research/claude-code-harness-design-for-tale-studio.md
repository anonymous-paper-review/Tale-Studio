...
- 진행 중 변경 폴더: `@../../../specs/changes/redesign-director-canvas/`
...
| 정보 종류 | 어디로 | 비고 |
|---|---|---|
| "노드 ID는 nanoid(10)" | `.claude/rules/director-canvas.md` (paths) + `src/features/director/CLAUDE.md` | 두 곳에 작성 안 하고, paths-scoped rule이 primary |
...
| `director-canvas.md` | `src/features/director/**` | React Flow `nodeTypes` module-scope, nanoid(10), 8px snap, 캔버스 토큰 |
...
#### `specs/changes/redesign-director-canvas/proposal.md`
...
## What Changes
(specs/layers/director_canvas.md 본문에서 추출하여 채움)

## Impact
- Affected specs: layers/director_canvas.md
- Affected code: src/features/director/ (이미 canvas-nodes/, canvas-edges/, canvas-popups/ 신설 중)
- Affected stores: stores/director-canvas-store.ts (이미 존재)
...
| `specs/layers/L*.md`, `specs/layers/director_canvas.md` | **그대로 유지** — source-of-truth. 변경은 changes/ 거쳐서. | OpenSpec 패턴 |
...
- source-of-truth: `specs/layers/director_canvas.md`
- 변경 폴더: `specs/changes/redesign-director-canvas/` (NEW)
- 구조 동일
