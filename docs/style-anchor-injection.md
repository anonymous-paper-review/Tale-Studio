...
> 2026-07-13 · 상태: **백엔드 구현·검증 완료** (ralplan→ultragoal; §8). UI 피커 도입됨(producer 그리드). A/B: Q1/Q4 ✅, Q3 → **2026-07-14 최종 판정: 범인은 art_style 슬롯의 매체어 — 앵커 존재 시 art_style 토큰 억제 적용** (`docs/style-anchor-art-style-authority.md` §9).
...
> **⚠️ 2026-07-14 재판정(2단계) — 최종.** ① Q3 은 실제 데이터엔 없는 문장형 매체 선언(`art style: photorealistic live-action`)으로 돌린 것이라, 실 토큰 조합(noir + bare `texture: photorealistic`)에선 앵커가 이김(오전 Q3b). ② 그러나 실 사고(d6208bba, us_cartoon 앵커 + `art style: dark_cinematic_realism` → 실사 출력)로 **art_style 슬롯에 매체어가 실리면 앵커가 지는 것**이 실측 확정(오후 exp4). 수정: 앵커 존재 시 generate-sheet 가 art_style 토큰만 억제(C2r 검증) — texture/line/shape/palette 유지. 상세: `docs/style-anchor-art-style-authority.md` §9.
