...
- ~~질문: **캡 완화 방식** — decoupage.ts:52-53(A1) 완화 시 C1(예시)·D1(게이팅)·B1(동시 금지)·D3(Compact)을 동반 수정할지. 캡만 풀면 예시 편향이 분포를 계속 누를 공산~~ → **닫힘 (2026-08-11, 실험 3호 `camera-contract-relax`)**: **A1 단독 완화로 충분.** 완화 문구("기본값 없이 내용이 요구하는가로 정한다 / 금지되는 건 무빙이 아니라 동기 없는 무빙")로 A1만 교체 → 감독 단계 무빙 14.93%→34.07%(×2.28, 3회 비중첩)가 **하류 v4까지 86% 전달**(격차 19.1%p→16.5%p) — 구조 관찰 "6겹 중 하나 풀어도 다섯 겹 잔존"은 **이 축에서 반증**. 카메라 type 어휘 7종→12종 실사용 개방, 부작용 없음(샷 수 +0.67·길이 −0.08s). 타깃팅 성립: 모션 요구 비트 적중 32.70%→66.07%(+33.37%p) ≫ 정적 비트 오발 +15.96%p. **단 사전 등록 기각②(오발 +15%p 이내) 0.96%p 차 발동 → 판정 "문구 v2 재설계"**(회차별 3회 중 2회는 임계 아래, merged 샷 겹침 제외 시 +12.40%p — 초과분 상당량이 계기 산물). 미배선(`WRITER_CAMERA_CONTRACT` 미설정=현행). 리포트: https://claude.ai/code/artifact/81449dc7-1399-46b2-9f4c-44664a751a68
- 질문(승계): **완화 문구 v2** — ① "프레임 안에서 완결되는 샷(인서트·리액션·대치)은 고정" 예시 강화 ② merged 샷(모션+정적 비트 동시 커버)은 고정 우선 명시 ③ 오발 지표에서 merged 겹침 분리. 닫히는 조건: v2 A/B 재실행(LLM only, 동일 규모).
...
- 실험 1·2호 종합 브리프 (readable-report 표준, 2026-08-11): https://claude.ai/code/artifact/f3e29ff0-9ed8-44e9-8a39-732b7354b0b0
