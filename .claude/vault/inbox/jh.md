...
사람이 쓴 원문 바이트는 고치거나 지우지 않는다. 밤 루프는 검증된 `night-runtime.py`로만 원문 주위에 `tracked`·`closed` 상태 표식을 붙인다. 일반 에이전트는 이 파일을 자유 편집하지 않는다. 실행을 시작할 때 원문과 상태를 함께 스냅샷하고 범위와 해시를 결과 기록에 남긴다.
...
<!-- vault-inbox-item:start
{"actor":"jh","byte_length":70,"content_sha256":"9e4850899fb5204aee16c71f56d5c19f84d398e6cab1902230ba66878e3e40a9","item_id":"648d14f5a88d3e3b3c7fac461daccddf84d0014da3211f706c27dee6d8e4e806","schema":1,"snapshot_id":"inbox-bootstrap-2026-08-18","source_key":"inbox/jh.md","source_range":{"end":5758,"start":5688},"state":"tracked","units":["deferred-regeneration-history"]}
-->
...
8. 다음밤 후보
 ...
 - 에이전트 얼굴 변경 : https://claude.ai/code/artifact/04f65efe-f395-4273-bd65-0d9fa54da3b4 이 아티팩트 참여해서 SD, LD 버전 애니메이션 3종 제작 (가만히 있을 때 웃거나 눈 깜빡임, 생각하는 중, 작업하는 중)
 - 여러 탭에 다른 프로젝트 : 현재 URL에 프로젝트 아이디가 적혀있긴 하지만 여러 브라우저 탭에 다른 프로젝트 띄우면 마지막에 띄운 프로젝트가 우선이 되어서 모든 브라우저 탭에서 에이전트 탭 이동을 하면 우선된 프로젝트가 보이게 되어있음 (여러 프로젝트 작업이 불가능함) 원인 분석 및 해결 방법 모색
 - 배경에 인물 안 나오게 : 바로 수정 진행
...
<!-- vault-inbox-item:start
{"actor":"jh","byte_length":576,"content_sha256":"121e05511582c409119f19f617c1a87126b13f55c7744e32c93c43df596a3186","item_id":"d8ef8af8e2af4237fac46eac0c4e76da7a7b31e1cc31b0c9f4862852cc857b51","schema":1,"snapshot_id":"inbox-bootstrap-2026-08-18","source_key":"inbox/jh.md","source_range":{"end":13644,"start":13068},"state":"tracked","units":["r4-progress-batch-scoped-count","audit-shot-cast-llm-failure-modes","deferred-regeneration-history","r14-chat-file-drag-drop"]}
-->
1. `6/8` 진행률은 채팅창 위 상단바에서 봤고, writer 러프·director 그림·director 영상 모두 같은 버그가 발생하므로 세 경로 전부 수정·검증한다.
2. 인물 연결은 최근 정상 동작하지만, 정상 사례만 통과한 것일 수 있다. LLM 파싱 오류가 생겼을 때 잘못된 연결을 저장하지 않고 안전하게 실패하는 장치가 있는지 확인한다.
3. 재생성 히스토리(버전 관리)는 추후 결정으로 보류한다.
4. 채팅창 외부 파일 드래그 앤 드롭은 지금 바로 수정한다.
<!-- vault-inbox-item:end -->
...
- 뮤직비디오는 그냥 뉴진스 hypeboy
- 광고는 코카콜라 영상 짧은거 (1분이내 있으면?)없으면 1분이상도상관없음 아무거나
- 웹툰은 아마 입력찾기가 힘들거라 웹툰 풀로넣기보다는 길게 스크롤된 웹툰 하나 어떻게든 긁어올수있으면 그걸로 실험해보면될듯?
2. 연속 프로젝트 만들때 다른 프로젝트에서 이전 프로젝트 상속받기(캐릭터, 월드, 이어질 전 이야기 등)
- 이건 그냥 상속에대한 스키마 정의해두고 db에서 가져와야하려나? 기술적인부분에 대한 인사이트가없어서 우리프로젝트구조보고 니가 제안하는방식으로 하면 될듯?
3. export 다양하게 테스트
- 사실 1번과 비슷한 논의긴함 컨텐츠의 종류, 입력의 확장자의 차이일뿐
...
글 작가에게 다시 시켜도 화면이 멈춰 있던 것 -> 승인
 새로고침하면 선택지와 올린 파일이 사라지던 것 -> 새로고침은 승인, 올린 파일이 사라진 것이 아니라 현행 유지
 채팅에서 특정 장면·컷을 콕 집어 부를 수 없던 것 -> 승인
 진행률이 예전에 끝난 작업까지 세던 것 -> 승인
 채팅창에 파일 끌어다 놓기 -> 승인
 확대·축소 단축키 -> 승인
 로그인 화면 대문자 잠금 표시 -> 승인
 배경 그림에 사람이 나오던 것 -> 승인
