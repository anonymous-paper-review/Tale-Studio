...
> 착륙(27ebaf3), migration 원격 적용·상태기계 실측 완료. **남은 것은 아래 이슈 현황·논의 항목의
> [열림]·[부분] 표시 참고 — 핵심은 B3 의도 분류(논의 #2) 설계 결정.**
...
|---|---|---|---|
| B1 | Artist | 체장수 나이를 젊은 옥화랑 맞춰달라고 함 → Artist가 그러겠다고 답함 → 이후 무반응 | 해소 — 승인 카드 + 상태 전이(awaiting→queued→completed) 실측 |
| B2 | Director | 채팅방에서 지정한 행동을 하지 않음 | 부분 — trace/실행 배선 완료, 실제 실행 정책은 논의 #1·#2에 걸림 |
| B3 | Director | 비디오가 아닌 수정사항을 얘기했으나 [p4] 등장하고 video take 추가하고 생성한다고 함 (당시 로그의 내부 표기 누출·의도 오해) — 결국 수정 방법을 못 찾음 | **열림 — 의도 분류(수정 vs 생성) 미결, 논의 #2** |
...
- 아직 연결하지 않은 경계: 승인 이후 비동기 생성 잡의 `job_id`를 최초 채팅 trace에 결합하는 것. 생성 잡 자체의 완료·실패 알림은 기존 경로가 담당하고, trace 결합은 2단계의 승인 정책과 4단계의 서버 액션 계약을 정한 뒤 진행한다.

## 논의 항목 (현황 2026-08-27)

1. **[부분·명문화 필요] 자동 실행의 경계**: 챗이 어디까지 스스로 실행해도 되나? (Take 차감이 있는 생성이라 과금이 걸림) 후보: (a) 무조건 실행, (b) "재생성할까요? [실행 버튼]" 카드 제시 후 원클릭, (c) 설정으로 선택. 오너 성향상 (b)가 유력해 보이나 확인 필요.
   → Artist 재생성은 (b) 원클릭 승인 카드로 착륙. **남은 결정: (b)를 전 스테이지 표준으로 확정할지, 과금 큰 Director 영상 생성도 동일하게 갈지(예상 Take 표시 등).**
2. **[열림] B3 의도 분류**: "수정사항 전달"과 "생성 요청"을 구분하는 의도 분류를 프롬프트로 풀지, UI(수정 모드 토글)로 풀지. → **미결. 이 그룹의 핵심 남은 설계 결정.**
...
- 챗에서 수정 요청 → 실행(또는 원클릭 실행 카드) → 진행 표시 → 완료 알림까지 전 스테이지에서 동작.
  → 현황: Artist 상태기계(승인→queued→completed) 실측 완료. **Director 영상·Writer 실제 E2E는 미검증(코드 배선만).**
- 실행 불가능한 요청에는 불가능하다고 답한다 (거짓 수락 금지).
  → 현황: B4 샷 체이닝은 미지원 기록으로 충족. **B3 의도 분류(#2)가 열려 있어 아직 완전하지 않음.**
