# 그룹 B — 챗이 "알겠다"고 하고 실행 안 함 (명령→실행 연결 끓김)

상태: **닫힘 (2026-08-31)** · 우선순위: 2

> 폐문(2026-08-31): 잔여 실측 전부 완주 — main 코드로 띄운 로컬 서버(:3100)에서 테스트 계정
> 픽스처 프로젝트로 발화 11개 실측, 11/11 의도 정분류(수정 즉시 7 · 생성 카드 정지 2 ·
> 미지원 정직 거절 1 · 되물음 1). B2 무과금 수정 즉시 적용(DB+trace) · B3 재현 검증+기준선 ·
> B5 Writer/Producer 챗 E2E 완주. 부수 잔여 2개(producer trace 배선 05c20f8 · chat_traces DB 타입)도 채움.
> 실측 중 발견·수리: **Writer 챗 대사 수정이 emotion·delivery·durationHint를 지우던 결함**(4c8c1fb,
> 컨텍스트 직렬화·프롬프트 예시·sanitizer 세 겹) — G7 영상 대사 절이 쓰는 필드다.
> 발행 리포트: `.claude/docs/2026-08-31/group-b-close-report.html`
...
| B2 | Director | 채팅방에서 지정한 행동을 하지 않음 | **해소(2026-08-31 실측)** — 무과금 카메라 수정 즉시 적용(DB vertical=3 · trace applied=1) |
| B3 | Director | 비디오가 아닌 수정사항을 얘기했으나 [p4] 등장하고 video take 추가하고 생성한다고 함 (당시 로그의 내부 표기 누출·의도 오해) — 결국 수정 방법을 못 찾음 | **해소(2026-08-31 재현 검증)** — 원문형 발화(손 떨림·카메라 흔들림)에 영상 시도 없이 무과금 수정 2건 적용. 기준선 11/11 정분류 |
| B4 | Director | 샷 체이닝을 챗방에 말했으나 알겠다고 하고 반영 안 됨 | 해소 — 미지원 액션으로 정직하게 기록(동의만·실행 안 함) |
| B5 | 전체 | 채팅방에서 수정하면 그게 이미지 재생성으로 연계되어야 함 (모르는 사람은 뭐 해야 할지 전혀 모름) | **해소** — Artist(08-27 E2E) · Director 이미지(08-28 배포 + 08-31 카드 재확인) · Writer·Producer(08-31 E2E). 영상은 의도된 미지원(노드 배선 브랜치 몹) |
...
## 남은 부수 요소 (커밋 27ebaf3에서 제외) — **채움 (2026-08-31, 05c20f8)**

- `applyExtractedSettings`는 이제 `applied/pending/rejected/noop`을 반환하고 traceId를 제안에 싣는다.
  승인 카드로 간 것을 `applied=1`로 적던 거짓 영수증 제거 — 화면 실측으로 pending→skipped 확인.
- `database.ts`에 live 스키마와 일치하는 `chat_traces` · `generation_jobs.chat_trace_id` 타입 추가.
  (`supabaseAdmin`에 Database 제네릭을 입히는 건 전역 라우트 영향이 커서 별건으로 남긴다.)
