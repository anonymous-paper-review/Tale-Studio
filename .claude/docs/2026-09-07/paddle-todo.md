...
## 다음 할 일 (2026-09-08 아침 기준, 순서대로)

결제 회로는 dev 에서 한 바퀴 돌았다(결제창 → Paddle → 웹훅 → 적립 → 화면 · 포털 취소). 라이브까지 남은 것:

| 순서 | 할 일 | 누가 | 왜 지금 |
|---|---|---|---|
| 1 | dev 스모크 6~12 (환불 · 부분 환불 · 취소 예약/철회/즉시 · 갱신 실패/복구) → `paddle-dev-smoke.md` ○/× | 오너 | 환불·갱신 실패 웹훅은 아직 Paddle 실물 페이로드를 못 봤다(보고서 3절 오른쪽). 여기서 필드 이름 오류가 잡힌다 |
| 2 | **P3 정책 문서 3종** 초안(약관 · 환불정책 · 개인정보) + 푸터 링크 | 에이전트 → 오너 문안 검수 | Paddle 라이브 심사 제출물. 없으면 라이브 키가 안 나온다 |
...
| 10 | 플랜 변경 화면(업그레이드: 그날 전액 결제 · 갱신일 리셋 · 남은 Take 이월) | 에이전트 | 첫 결제 뒤 |

main 푸시: 코드는 지금 올려도 된다. Production 에 Paddle 키·가격 ID 가 없어서 라이브에서는 결제 버튼이 전부 "준비 중"이고 웹훅은 호출될 일이 없다. 라이브에 보이는 변화는 실가격 페이지(베타 배너 유지) · 계정 탭 · nav Take 배지(잔액 0). 마이그레이션 없음.
...
## 2026-09-08 방향 변경 — Paddle = 해외 결제 전용

오너 확인: Paddle 로는 **해외 결제만** 받는다. 국내 결제는 이 축에 없다.

바뀌는 것

| 항목 | 전 | 후 |
|---|---|---|
| 정책 문서 3종 | 한국 전자상거래법 기준 + 국문 | **영문이 정본**. 해외 이용자가 읽는 문서 |
| 선불전자지급수단·소멸시효 검토 | 필수(법률 확인 대기) | **적용 축이 달라짐** — 한국 유저에게 안 팔면 이 규제 자리가 아니다. 세무사 확인 항목에서 성격 변경 |
| 가격 표기 | USD | USD 유지(변경 없음) |
| `/pricing` 한국어 | 결제 버튼 + 한국어 | **결제 버튼이 한국어 화면에 뜨는 게 맞는지 재검토 필요** |
| 세무 | 국내외 혼재 | 해외 매출 단일 축 |

확인 필요 (오너)
- Paddle Checkout settings 에서 국가·결제수단을 어떻게 제한했는지 (한국 카드 차단 여부)
- 국내 결제를 나중에 별도 PG 로 붙일 계획인지, 아예 안 받을 것인지
  → 이 답에 따라 `/pricing` 한국어 화면의 결제 버튼 처리가 갈린다
