...
| 3 | **만료 잡(2-6)**: 결제 주기 끝에 플랜 Take 를 지우는 Cron | 에이전트 | 지금은 장부에 만료일만 적혀 있고 안 사라진다. 첫 라이브 결제 전 필수 |
| 4 | **이중 적립 DB 제약**(보고서 6절 3번): `take_ledger(ref_kind, ref_id, kind)` 부분 유일 인덱스 | 에이전트 | 동시 재전송 경쟁을 조회가 아니라 DB 가 막게. 마이그레이션 1개(dev 먼저 → live) |
| 5 | **일일 대사(P12)**: Paddle 거래 목록 vs billing_events, 불일치 → 디스코드 | 에이전트 | 알림 유실 감지. Vercel Cron |
