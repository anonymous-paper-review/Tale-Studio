...
COUNT_LEAVES = [  # 개수·단계·가닥·돌기·부호·조각 — §4 14항 대상
    ...
    ('재질', '헤어', '구축'), ('재질', '헤어', '외곽'), ('인물', '헤어 구축'), ('인물', '눈', '디테일'), ('인물', '눈', '속눈썹'), ('인물', '눈', '공막'), ('인물', '눈', '하이라이트', '개수'),
...
def main():
    ...
    if re.search(r'홍채[^;]{0,80}판독 불가|판독 불가[^;]{0,40}홍채', lim):  # v1.2.2 사이클 7: '홍채 다단계·속눈썹 가닥·원경 이목구비는 판독 불가'처럼 ·로 이어진 목록도 잡는다
        ...
        # v1.2.2 사이클 7: 반대 방향도 모순 — 판독 불가라 해 놓고 홍채 그라데이션·하단 반사광·동공을 [실측] 없음/플랫/1색으로 적으면 '판독 불가 ≠ 없음'(§4 16항) 위반(refer4: 원작 홍채는 3단+동공인데 1색으로 적음)
        for leaf in (('인물', '눈', '홍채 렌더링', '그라데이션'), ('인물', '눈', '홍채 렌더링', '하단 반사광'), ('인물', '눈', '동공 처리'), ('인물', '눈', '처리')):
            v = get(data, leaf) or ''
            if v.startswith('[실측]') and re.search(r'없음|플랫|1색|단색|합쳐진', v): V('.'.join(leaf), "판독 한계가 홍채 단계 판독 불가인데 [실측] 없음/플랫/1색으로 적음 — 판독 불가 ≠ 없음(§4 16항): [추정]으로 낮추거나 '판독 불가'로 적는다", v)
