// 개발 DB의 격리 공간에서 Director 예약과 Take 원장 함수 원본을 실행하고 총 차감량을 기록한다.
...
const schema = `director_charge_${randomUUID().replaceAll('-', '')}`
const output = resolve(root, '.claude/docs/2026-09-14/button-gating/billing-verification.json')
const result = { startedAt: new Date().toISOString(), developmentProject: ref, schema,
  ...
  scope: 'Original SQL in isolated PostgreSQL schema; API replay-to-hold guard is separately source-inspected, not simulated here.',
  observations: [] }
const sources = {
  reserve_director_video_take: '20260720044000_director_video_metadata_identity.sql',
  reserve_director_video_regeneration: '20260720043600_director_video_retakes_scoped_replay.sql',
  ...
  take_hold: '20260909130000_fix_refund_expiry.sql',
...
try {
  ...
  if (!/^director_charge_[a-f0-9]{32}$/.test(schema)) throw new Error('Unsafe schema identifier')
  ...
  save()
  console.log(JSON.stringify({ stage: 'original SQL loaded into isolated development schema', functions: result.functions.length }))
  const status = await new Promise((done, reject) => {
    const child = spawn(process.execPath, ['node_modules/vitest/vitest.mjs', 'run', 'tests/manual/director-double-charge.manual.test.ts', '--reporter=verbose'], {
      cwd: root, stdio: ['ignore', 'pipe', 'pipe'], timeout: 180000,
      env: { ...process.env, DIRECTOR_CHARGE_DATABASE_URL: connectionText, DIRECTOR_CHARGE_CA: ca, DIRECTOR_CHARGE_SCHEMA: schema, DIRECTOR_CHARGE_OUTPUT: output },
    })
