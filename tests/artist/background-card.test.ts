...
// capacityReservationRejection: 예약 선행 경로의 트리거 자리 거절만 429 로 옮긴다 — 이 파일은 그 경로를 다루지 않아 통과(null)로 둔다.
vi.mock('@/lib/api/quota', () => ({ quotaRejectionResponse: () => new Response('quota', { status: 429 }), capacityReservationRejection: () => null }))
