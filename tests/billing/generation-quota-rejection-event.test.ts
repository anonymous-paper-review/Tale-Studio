...
import { quotaRejectionResponse, capacityReservationRejection } from '@/lib/api/quota'
import type { QuotaCheck } from '@/lib/generation-quota'

const userBlocked: QuotaCheck = { ok: false, queued: 6, limit: 6, scope: 'user', category: 'image', axis: 'user_image' }
const globalBlocked: QuotaCheck = { ok: false, queued: 18, limit: 18, scope: 'global', category: 'video', axis: 'global' }
...
describe('예약 경쟁에서 한도에 걸리면 대기 안내를 보내는 약속', () => {
  ...
  it.each([
    ...
    const response = capacityReservationRejection(error, context)
    ...
    expect(recordMock).toHaveBeenCalledWith('proj-1', 'generation_submit_rejected_quota', {
      kind: 'shot_video', scope: 'user', axis: 'user_video', queued: 4, limit: 3, userId: 'user-1',
    })
  })
  ...
  it.each(['3junk', '3.5', '3e2', '9007199254740993', '2', '', undefined])(
    ...
    (details) => {
      ...
      expect(capacityReservationRejection(error, context)).toBeNull()
