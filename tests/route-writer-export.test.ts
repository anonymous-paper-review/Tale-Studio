...
describe('GET /api/writer/export/[projectId]', () => {
  ...
  // #writer-overhaul 2026-08-10: v5(renderPrompts) 스테이지 제거 후의 등가성.
  //   구 run 은 state.renderPrompts 를 그대로(과거 export 와 바이트 동일), 신규 run 은
  //   shotSequence 에서 v5 와 같은 우선순위로 파생해 prompts.md 가 비지 않게 한다.
  it('derives render prompts from shotSequence when the run predates no v5 stage output', async () => {
    mocks.getUser.mockResolvedValue(USER)
    mocks.userOwnsProject.mockResolvedValue(true)
    const shotSequence = {
      shots: [
        {
          ...
          duration_seconds: 5,
          S: { scene_id: 'scene_1' },
          first_frame_generation: { composition_prompt: 'A close-up of an old box.' },
          video_generation: { motion_prompt: 'Camera slowly pushes in.' },
        },
        {
          // 조립 출력이 빈 샷 — v5 와 동일하게 v4 스펙 원본으로 폴백한다.
          shot_id: 'shot_2',
          duration_seconds: 3,
          S: { scene_id: 'scene_1' },
          static_spec: { first_frame_prompt: 'The lid rests open.' },
          dynamic_spec: { motion_prompt: 'Dust settles.' },
        },
      ],
    }
    mockWriterRuns([
      run('completed', { genre, narrativeStructure, characters, scenes, shotDesign, shotSequence }, '2026-08-10T00:00:00Z'),
    ])
    ...
    expect(response.status).toBe(200)
    expect(body.renderPrompts).toEqual({
      total_shots: 2,
      shots: [
        {
          shot_id: 'shot_1',
          scene_id: 'scene_1',
          duration_seconds: 5,
          t2i: { prompt: 'A close-up of an old box.' },
          ti2v: { motion_prompt: 'Camera slowly pushes in.', duration_seconds: 5 },
        },
        {
          shot_id: 'shot_2',
          scene_id: 'scene_1',
          duration_seconds: 3,
          t2i: { prompt: 'The lid rests open.' },
          ti2v: { motion_prompt: 'Dust settles.', duration_seconds: 3 },
        },
      ],
    })
  })

  it('keeps the stored v5 output for legacy runs that still carry it', async () => {
    mocks.getUser.mockResolvedValue(USER)
    mocks.userOwnsProject.mockResolvedValue(true)
    const shotSequence = {
      shots: [{ shot_id: 'shot_1', S: { scene_id: 'scene_1' }, video_generation: { motion_prompt: 'derived instead' } }],
    }
    mockWriterRuns([run('completed', { ...completedState, shotSequence }, '2026-07-01T00:00:00Z')])

    const response = await GET(request(), ctx())
    const body = await response.json()

    expect(body.renderPrompts).toEqual(renderPrompts)
  })
