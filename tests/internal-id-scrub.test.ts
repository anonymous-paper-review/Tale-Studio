...
//   보이지 않는다는 계약. 정책: 샷 규칙 생성명 "씬2 · 샷7"(EN "Scene 2 · Shot 7"),
//   내부 id 는 UI 전면 금지(디버그 예외 없음).

describe('shot/scene 표시명 (#internal-id-scrub)', () => {
  it('sh_02_07 → 씬2 · 샷7 / Scene 2 · Shot 7 (0패딩 제거)', () => {
    expect(shotIdDisplayName('sh_02_07', 'ko')).toBe('씬2 · 샷7')
    expect(shotIdDisplayName('sh_02_07', 'en')).toBe('Scene 2 · Shot 7')
    expect(shotIdDisplayName('sh_4_31', 'ko')).toBe('씬4 · 샷31')
  })

  it('sc_04 → 씬4 / Scene 4, 패턴 밖은 null', () => {
    expect(sceneIdDisplayName('sc_04', 'ko')).toBe('씬4')
    expect(sceneIdDisplayName('sc_04', 'en')).toBe('Scene 4')
    expect(shotIdDisplayName('char_2', 'ko')).toBeNull()
    expect(sceneIdDisplayName('scene-x', 'ko')).toBeNull()
  })
})

describe('산문 스크럽 — 챗 말풍선 최종 방어', () => {
  it('산문 속 샷·씬 id 를 표시명으로 치환한다', () => {
    expect(scrubInternalIdsInProse('sh_02_07 의 구도를 sc_04 톤에 맞췄어요.', 'ko')).toBe(
      '씬2 · 샷7 의 구도를 씬4 톤에 맞췄어요.',
    )
  })

  it('[p3] 마커는 걷고 [L3] 스크립트 라인 참조는 보존한다', () => {
    expect(scrubInternalIdsInProse('[p3] 문단과 [L3] 라인을 참고했어요.', 'ko')).toBe(
      '문단과 [L3] 라인을 참고했어요.',
    )
  })
  ...
  it('이름 맵이 오면 char/loc id 도 이름으로 바꾼다', () => {
    ...
    expect(
      scrubInternalIdsInProse('char_new_5l6sj 를 loc_riverside 에 배치했어요.', 'ko', names),
