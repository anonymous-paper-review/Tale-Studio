...
export async function runL4Shots(
  ...
  l1: L1Style,
  l2: L2Design,
  scenePlans: L3SceneVisualPlan[] | null,  // null이면 Compact Mode
  logger: PipelineLogger,
  axisConfig: LlmAxisConfig,
): Promise<L4Shot[]> {
  const compactMode = scenePlans === null;
  await logger.markStage('L4_shots', 'started', { compact_mode: compactMode });

  // 씬별로 L4 생성 (씬 디시플린 명확하게 적용하기 위해 분리 호출)
  const allShots: L4Shot[] = [];
  for (const scene of s3.scenes) {
    const plan = compactMode ? null : scenePlans!.find((p) => p.scene_id === scene.scene_id) ?? null;
    if (!compactMode && !plan) {
      console.warn(`[L4] no L3 plan for ${scene.scene_id}, skipping`);
      continue;
    }
    const sceneShots = await generateL4ForScene(scene, plan, s0, s2, l1, l2, logger, axisConfig);
    allShots.push(...sceneShots);
  }

  await logger.saveStage('11_L4_shots.json', { shots: allShots, compact_mode: compactMode });
  await logger.markStage('L4_shots', 'completed', { shot_count: allShots.length, compact_mode: compactMode });
...
async function generateL4ForScene(
  scene: S3Scene,
  plan: L3SceneVisualPlan | null,    // null = Compact Mode (L3 미제공)
  s0: S0Genre,
  s2: S2Block,
  l1: L1Style,
  l2: L2Design,
...
- 시선/180°축은 L3.spatial_axis_180 준수`;
  ...
  const systemInstruction = `당신은 V축 L4(샷 실행) 디자이너이다.
한 씬 안의 모든 샷을 생성한다.
...
${compactMode
    ? '[L3 미제공 — Compact Mode. L4가 자체적으로 디시플린 결정]'
    : `[이 씬의 L3 비주얼 플랜 — 반드시 준수]\n${JSON.stringify(plan, null, 2)}`}
...
[이 씬 등장 캐릭터 상세]
${JSON.stringify(
  s2.characters.filter((c) => scene.characters_in_scene.includes(c.id))
)}
...
locations=${JSON.stringify(l2.locations.filter((loc) => loc.id === scene.location || scene.location.includes(loc.id)))}
costumes=${JSON.stringify(
    Object.fromEntries(
      scene.characters_in_scene
        .map((cid) => [cid, l2.costumes[cid]])
...
[출력 형식 - JSON]
{
  ...
  if (Array.isArray(r)) {
    if (r.length === 1 && r[0] && typeof r[0] === 'object' && 'shots' in r[0]) {
      shots = (r[0] as { shots: L4Shot[] }).shots;
    ...
    if (inner.length > 0 && inner[0] && typeof inner[0] === 'object' && 'shots' in (inner[0] as object) && !('intent' in (inner[0] as object))) {
      shots = (inner[0] as { shots: L4Shot[] }).shots;
    } else {
      shots = inner as L4Shot[];
    }
  } else {
  ...
  // shot_id 표준화 (전역 인덱싱은 호출부에서)
  return shots.map((shot, i) => {
    const sid = shot.intent.shot_id ?? `shot_${scene.scene_id}_${String(i + 1).padStart(3, '0')}`;
    return {
