...
import { PipelineLogger, makeProjectId } from '@/lib/logger';
import { runS0 } from '@/lib/pipeline/stages/s0_genre';
import { runS1 } from '@/lib/pipeline/stages/s1_structure';
import { runS2 } from '@/lib/pipeline/stages/s2_characters';
import { runS3 } from '@/lib/pipeline/stages/s3_scenes';
import { runCValidation1 } from '@/lib/pipeline/stages/c_validation_1';
import { runMidPreview } from '@/lib/pipeline/stages/mid_preview';
import { runL0L1 } from '@/lib/pipeline/stages/l0_l1_visual';
import { runL2 } from '@/lib/pipeline/stages/l2_design';
import { runL3SceneVisualPlan } from '@/lib/pipeline/stages/l3_scene_plan';
import { runL4Shots } from '@/lib/pipeline/stages/l4_shots';
import { runCApplication2 } from '@/lib/pipeline/stages/c_application_2';
import { runL5Prompts } from '@/lib/pipeline/stages/l5_prompts';
import { inferL3FromL4Shots } from '@/lib/pipeline/util/infer_l3';
import { isCompactDepth } from '@/lib/types/pipeline';
import { analyzeSceneActionBudget } from '@/lib/pipeline/validators/action_budget';
import { resetGeminiCallCount, getGeminiCallCount } from '@/lib/llm/gemini';
import { resetClaudeCallCount, getClaudeCallCount } from '@/lib/llm/claude';
import { resetOpenAICallCount, getOpenAICallCount } from '@/lib/llm/openai';
import { resetLocalCallCount, getLocalCallCount } from '@/lib/llm/local';
import { resetRawSeq } from '@/lib/llm/raw_collector';
import { DEFAULT_MODELS, type PipelineModelsConfig, type LlmAxisConfig } from '@/lib/llm/dispatch';
import type {
  PipelineInput,
  PipelineResult,
  L3SceneVisualPlan,
...
export async function runPipeline(
  ...
  const projectId = resumeProjectId ?? makeProjectId();
  const logger = new PipelineLogger(projectId);
  await logger.init();
  const isResume = Boolean(resumeProjectId);
  ...
  if (isResume) {
    await logger.markStage('PIPELINE', 'started', { resumed: true });
  }
...
async function _runPipelineInner(
  ...
  // L4: 샷 단위 3분할 (intent + static + dynamic). Compact mode에선 L3 미제공.
  const l4Result = await loadOrRun<{ shots: L4Shot[]; compact_mode: boolean }>(
    resume,
    '11_L4_shots.json',
    async () => {
      const shots = await runL4Shots(S0, S2, S3, L1, L2, compact ? null : L3, logger, models.V);
