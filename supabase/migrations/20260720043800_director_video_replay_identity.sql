...
-- A new take is one operation per (project, idempotency key).  Lock that
-- operation before locking its shot so a same-key cross-shot race always sees
-- the original reservation and returns an idempotency conflict.
...
declare
  v_clip public.video_clips%rowtype;
  v_job uuid;
  v_take integer;
  v_existing_snapshot jsonb;
  v_existing_target jsonb;
  v_existing_take_label text;
  v_existing_override jsonb;
  v_existing_canvas_position jsonb;
  v_snapshot jsonb;
...
begin
  ...
  v_metadata := jsonb_build_object(
    'take_label', p_take_label,
    'override', coalesce(p_override, '{}'::jsonb),
    'canvas_position', p_canvas_position
  );
  v_snapshot := coalesce(p_input_snapshot, '{}'::jsonb) || jsonb_build_object(
    'requestedModel', p_model,
    'new_take_metadata', v_metadata
  );
  if v_snapshot->'new_take_metadata' is distinct from v_metadata then
  ...
  select p.workspace_id into v_workspace_id
    from public.projects p join public.shots s on s.project_id = p.id
  ...
  select gj.video_clip_id, gj.id, gj.input_snapshot, gj.target,
         vc.take_label, vc.override, vc.canvas_position
    into video_clip_id, job_id, v_existing_snapshot, v_existing_target,
         v_existing_take_label, v_existing_override, v_existing_canvas_position
    from public.generation_jobs gj
    join public.video_clips vc on vc.id = gj.video_clip_id and vc.project_id = gj.project_id
    where gj.project_id = p_project_id and gj.kind = 'shot_video'
      and gj.idempotency_key = p_idempotency_key and gj.target->>'retakeMode' = 'new_take'
    limit 1;
  if found then
    ...
    if v_existing_target->>'writerShotId' is distinct from p_shot_id
      ...
      or v_existing_take_label is distinct from p_take_label
      or v_existing_override is distinct from coalesce(p_override, '{}'::jsonb)
      or v_existing_canvas_position is distinct from p_canvas_position then
  ...
  select coalesce(max(vc.take_number), 0) + 1 into v_take
    from public.video_clips vc where vc.project_id = p_project_id and vc.shot_id = p_shot_id;
  insert into public.video_clips(project_id, shot_id, take_number, status, last_attempt_status, last_attempt_at, updated_at, take_label, override, canvas_position)
    values (p_project_id, p_shot_id, v_take, 'pending', 'generating', now(), now(), p_take_label, coalesce(p_override, '{}'::jsonb), p_canvas_position)
  ...
  insert into public.generation_jobs(id, project_id, request_id, model, kind, status, target, input_snapshot, video_clip_id, idempotency_key, user_id, workspace_id, provider, actor)
    values (v_job, p_project_id, 'reserved:' || v_job::text, p_model, 'shot_video', 'queued', coalesce(p_target, '{}'::jsonb) || jsonb_build_object('videoClipId', v_clip.id), v_snapshot, v_clip.id, p_idempotency_key, p_user_id, v_workspace_id, coalesce(p_provider, 'fal'), coalesce(p_actor, 'ui'));
