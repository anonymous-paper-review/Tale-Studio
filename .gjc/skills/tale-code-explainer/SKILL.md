...
```text
overview, architecture, flow, code-journey, concepts, tradeoffs, check, evidence
```

`concepts`는 `verify-guide.mjs`가 검사하지 않는다(검사기 계약은 오너 테스트 `tests/ops/tale-code-explainer.test.ts`가 고정하고 있어 이 스킬이 바꾸지 않는다). 검사기가 안 잡아도 이 절은 필수다.
...
독자는 TypeScript와 Next.js가 처음이다. 다음 셋은 형식이 아니라 이해 가능성의 최소선이라 빠뜨리지 않는다.

- **줄 각주**: 코드 블록마다 원문을 건드리지 않고 줄 번호로 키를 맞춘 각주 표를 붙인다. 처음 보는 기호(`?.` `??` `!` `=>` 등), 낯선 문장 모양, 오해하기 쉬운 이름, 그 줄이 없으면 생기는 일을 한 줄씩 적는다.
- **배경 분류**: 각 카드가 쓰는 방식을 `업계 통용`·`프레임워크 강제`·`이 프로젝트 선택` 중 하나로 표시한다. 독자가 "그냥 이렇게 하는구나"에서 멈추지 않게 한다.
- **다음 수와 오답**: tradeoffs에서 알려진 위험을 적은 행은 `대책 있음`·`안고 간다`·`근거 미확인` 중 하나를 밝히고, check의 각 답에는 가장 나오기 쉬운 오답과 그게 틀린 이유를 코드 근거로 적는다.
- **개념 모델**: 코드 설명만으로는 재사용이 안 된다. 추적 경로에 실제 등장한 개념 최대 5개를 골라 `없던 시절의 문제 → 해법 아이디어 → 이 저장소의 증거 → 법칙/관습/선택 → 안 통하는 경계 → 전이 질문` 순으로 쓴다. 개념을 먼저 설명하고 코드를 예시로 붙이지 않는다 — 순서는 항상 구체 코드에서 개념으로 올라가는 귀납이다.

상세 규칙은 `references/granularity.md`와 `references/curriculum.md`를 따른다.
